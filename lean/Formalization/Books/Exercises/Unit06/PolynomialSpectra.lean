...
theorem integer_polynomial_maximal_ideal_classification
  ...
  constructor
  · intro hI
    have hZjac : IsJacobsonRing ℤ := by
      rw [isJacobsonRing_iff_prime_eq]
      intro Q hQ
      obtain hzero | ⟨p, hp, hpQ⟩ := (Ideal.isPrime_int_iff (P := Q)).mp hQ
      · rw [hzero, Ideal.jacobson_bot, Ring.jacobson_eq_sInf_isMaximal]
        apply le_antisymm
        · intro x hx
          by_contra hx0
          obtain ⟨q, hqle, hq⟩ := Nat.exists_infinite_primes (x.natAbs + 1)
          have hqgt : x.natAbs < q := lt_of_lt_of_le (Nat.lt_succ_self _) hqle
          have hqmax : (Ideal.span {(q : ℤ)}).IsMaximal :=
            @Int.ideal_span_isMaximal_of_prime q ⟨hq⟩
          have hqx : x ∈ Ideal.span {(q : ℤ)} :=
            (Ideal.mem_sInf.mp hx) hqmax
          have hqdiv : q ∣ x.natAbs :=
            Int.natCast_dvd.mp (Ideal.mem_span_singleton.mp hqx)
          exact (not_le_of_gt hqgt)
            (Nat.le_of_dvd (Int.natAbs_pos.mpr hx0) hqdiv)
        · exact bot_le
      · rw [hpQ]
        let _ : Fact (Nat.Prime p) := ⟨hp⟩
        exact Ideal.jacobson_eq_self_of_isMaximal
    let _ : IsJacobsonRing ℤ := hZjac
    let Q : Ideal ℤ := I.comap (Polynomial.C : ℤ →+* integerPolynomialRing)
    have hQprime : Q.IsPrime := by
      exact Ideal.comap_isPrime (Polynomial.C : ℤ →+* integerPolynomialRing) I
    have hQne : Q ≠ ⊥ := by
      intro hQ
      have hQmax : Q.IsMaximal := by
        dsimp [Q]
        let _ : I.IsMaximal := hI
        apply Polynomial.isMaximal_comap_C_of_isMaximal I
        intro z hz
        have hzQ : z ∈ Q := hz
        rw [hQ] at hzQ
        exact hzQ
      have hfield : IsField ℤ :=
        (Ring.isField_iff_maximal_bot (R := ℤ)).mpr (hQ ▸ hQmax)
      exact Int.not_isField hfield
    obtain hzero | ⟨p, hp, hpQ⟩ := (Ideal.isPrime_int_iff (P := Q)).mp hQprime
    · exact (hQne hzero).elim
    let _ : Fact (Nat.Prime p) := ⟨hp⟩
    let φ : integerPolynomialRing →+* Polynomial (ZMod p) :=
      Polynomial.mapRingHom (Int.castRingHom (ZMod p))
    have hsurj : Function.Surjective φ := by
      exact Polynomial.map_surjective _ (ZMod.ringHom_surjective _)
    have hker : RingHom.ker φ ≤ I := by
      dsimp [φ]
      rw [Polynomial.ker_mapRingHom, ZMod.ker_intCastRingHom]
      apply Ideal.map_le_iff_le_comap.mpr
      rw [← hpQ]
    let J : Ideal (Polynomial (ZMod p)) := I.map φ
    have hJmax : J.IsMaximal := by
      dsimp [J]
      let _ : I.IsMaximal := hI
      exact Ideal.IsMaximal.map_of_surjective_of_ker_le hsurj hker
    have hJne : J ≠ ⊥ := by
      intro hJ
      have hfield : IsField (Polynomial (ZMod p)) :=
        (Ring.isField_iff_maximal_bot (R := Polynomial (ZMod p))).mpr (hJ ▸ hJmax)
      exact Ideal.polynomial_not_isField hfield
    obtain hJbot | ⟨g, hg, hJg⟩ :=
      (Ideal.isPrime_iff_of_isPrincipalIdealRing_of_noZeroDivisors).mp hJmax.isPrime
    · exact (hJne hJbot).elim
    refine ⟨p, hp, g, hg.irreducible, ?_⟩
    have hImap : (I.map φ).comap φ = I := by
      rw [Ideal.comap_map_of_surjective φ hsurj, ← RingHom.ker_eq_comap_bot,
        sup_eq_left.mpr hker]
    calc
      I = (I.map φ).comap φ := hImap.symm
      _ = (Ideal.span {g}).comap φ := by
        simpa [J] using congrArg (Ideal.comap φ) hJg
      _ = integerPolynomialMaximalIdeal p g := rfl
  · rintro ⟨p, hp, g, hg, rfl⟩
    let _ : Fact (Nat.Prime p) := ⟨hp⟩
    let _ : (Ideal.span {g} : Ideal (Polynomial (ZMod p))).IsMaximal :=
      PrincipalIdealRing.isMaximal_of_irreducible hg
...
theorem integer_polynomial_spectrum_prime_ideals :
  ...
  intro P
  by_cases hpbot : P.asIdeal = (⊥ : Ideal integerPolynomialRing)
  · exact Or.inl hpbot
  right
  by_cases hpmax : P.asIdeal.IsMaximal
  · exact Or.inr hpmax
  let _ : P.asIdeal.IsPrime := P.2
  have hdim : ringKrullDim integerPolynomialRing = (2 : WithBot ℕ∞) := by
    rw [Polynomial.ringKrullDim_of_isNoetherianRing,
      IsPrincipalIdealRing.ringKrullDim_eq_one ℤ Int.not_isField]
    norm_num
  let _ : FiniteRingKrullDim integerPolynomialRing :=
    finiteRingKrullDim_iff_ne_bot_and_top.mpr ⟨by
      rw [hdim]
      exact WithBot.coe_ne_bot, by
      rw [hdim]
      ...
      intro h
      exact (ENat.natCast_ne_top 2) (WithBot.coe_eq_top.mp h)⟩
  have hle' : (P.asIdeal.height : WithBot ℕ∞) ≤ ringKrullDim integerPolynomialRing :=
    Ideal.height_le_ringKrullDim_of_ne_top (I := P.asIdeal) Ideal.IsPrime.ne_top'
  rw [hdim] at hle'
  have hle : P.asIdeal.height ≤ 2 := WithBot.coe_le_coe.mp hle'
  have hzero : P.asIdeal.height ≠ 0 := by
  ...
  have hprime : (Ideal.span {f}).IsPrime := by
    rw [show Ideal.span {f} = P.asIdeal by simpa using hf.symm]
    exact inferInstance
  exact Or.inl ⟨f, ((Ideal.span_singleton_prime hf0).mp hprime).irreducible, hf⟩
