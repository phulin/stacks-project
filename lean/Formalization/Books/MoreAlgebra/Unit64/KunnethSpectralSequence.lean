...
abbrev Comp (R : Type u) [CommRing R] := Formalization.Books.MoreAlgebra.Unit59.Comp R

/-- Filtered integer-indexed cochain complexes of `R`-modules. -/
abbrev FilteredComp (R : Type u) [CommRing R] :=
  Formalization.Books.Homology.Unit24.FilteredComplex (Mod R)

/-- The unfiltered complex underlying a filtered complex. -/
noncomputable abbrev forgetFiltered {R : Type u} [CommRing R]
    (K : FilteredComp R) : Comp R :=
...
noncomputable abbrev tensorUnderlying
    {R : Type u} [CommRing R] (K L : FilteredComp R) : Comp R :=
  Formalization.Books.MoreAlgebra.Unit58.tensorProductComplex R (forgetFiltered K) (forgetFiltered L)
...
def FilteredComplexFlat
  ...
  (∀ n : ℤ, Module.Flat R ((K.X n).carrier : Type u)) ∧
    ...
    (∀ n p : ℤ, Module.Flat R (gradedPiece (C := Mod R) (K.X n) p : Type u))
...
noncomputable abbrev gradedTensorSummand
  ...
  tensorProductComplex R
    (Formalization.Books.Homology.Unit24.filteredComplexUnshiftedGradedPiece K i)
    (Formalization.Books.Homology.Unit24.filteredComplexUnshiftedGradedPiece L j)
...
structure TensorProductFiltrationSequence
  ...
  projection : Formalization.Books.Homology.Unit20.filteredComplexStepComplex (tensorFilteredComplex K L) n ⟶
    gradedTensorSum K L n
  projection_zero :
    Formalization.Books.Homology.Unit20.filteredComplexStepInclusion (tensorFilteredComplex K L) n (n + 1) (by omega) ≫
        projection = 0
  exact :
    (ShortComplex.mk
      (Formalization.Books.Homology.Unit20.filteredComplexStepInclusion
...
theorem tensorProduct_associatedGraded_sum
    ...
    Nonempty
      (gradedTensorSum K L n ≅
        Formalization.Books.Homology.Unit24.filteredComplexGradedPiece (tensorFilteredComplex K L) n) := by
  sorry
...
def KunnethFiniteFiltration
  ...
  Formalization.Books.Homology.Unit24.FilteredComplexFiniteFiltration K ∧
    Formalization.Books.Homology.Unit24.FilteredComplexFiniteFiltration L

/-- A filtration is eventually acyclic above. -/
def KunnethAcyclicAbove
    {R : Type u} [CommRing R] (K : FilteredComp R) : Prop :=
  ∃ b : ℤ, ∀ i : ℤ, b ≤ i →
    IsAcyclic (Formalization.Books.Homology.Unit24.filteredComplexFiltrationStep K i)
...
def KunnethQuasiIsoBelow
    {R : Type u} [CommRing R] (K : FilteredComp R) : Prop :=
  ∃ a : ℤ, ∀ i : ℤ, i ≤ a →
    QuasiIso (Formalization.Books.Homology.Unit20.filteredComplexStepToUnderlying K i)
    ...
    ∃ e : spectralSequence.page 1 (p, q) ≅ kunnethE₁Term K L p q,
      ∃ e' : spectralSequence.page 1 (p + 1, q) ≅ kunnethE₁Term K L (p + 1) q,
        KunnethD₁Support K L p q
          (e.inv ≫ spectralSequence.differential 1 (p, q) ≫
            (eqToHom (by sorry) :
              (Formalization.Books.Homology.Unit24.bigradedShift
                (1 : ℤ) (-(1 : ℤ) + 1)).obj
...
structure FilteredFreeKFlatResolution
  ...
  graded_free : ∀ n p : ℤ,
    Module.Free R (gradedPiece (C := Mod R) (complex.X n) p : Type u)
  complex_kflat : IsKFlat (forgetFiltered complex)
  filtration_kflat : ∀ p : ℤ,
    IsKFlat (Formalization.Books.Homology.Unit24.filteredComplexFiltrationStep complex p)
  graded_kflat : ∀ p : ℤ,
    IsKFlat (Formalization.Books.Homology.Unit24.filteredComplexUnshiftedGradedPiece complex p)
...
noncomputable abbrev boundedDerivedCohomology
    ...
    (K : Formalization.Books.MoreAlgebra.Unit56.DBounded R) (n : ℤ) : Mod R :=
  (derivedCohomologyFunctor (Mod R) n).obj
...
noncomputable abbrev boundedDerivedTensor
    {R : Type u} [CommRing R]
    [HasDerivedCategory.{w} (Mod R)]
    (K L : Formalization.Books.MoreAlgebra.Unit56.DBounded R) : Formalization.Books.MoreAlgebra.Unit56.D R :=
  derivedTensor
    ((DerivedCategory.Bounded.ι (C := Mod R)).obj K)
...
structure BoundedKunnethConvergence
    ...
    (K L : Formalization.Books.MoreAlgebra.Unit56.DBounded R) where
  eInf : ℤ × ℤ → Mod R
  filtration : ∀ n : ℤ,
  ...
  stabilizes : ∀ p q : ℤ, ∃ r₀ : ℤ, 2 ≤ r₀ ∧
    ∀ r : ℤ, ∀ hr : r₀ ≤ r,
      Nonempty ((E.page r (by sorry)).X (p, q) ≅ eInf (p, q))
  abutment : ∀ p q : ℤ,
...
theorem bounded_kunneth_spectral_sequence_exists
    {R : Type u} [CommRing R]
    [HasDerivedCategory.{w} (Mod R)]
    (K L : Formalization.Books.MoreAlgebra.Unit56.DBounded R) :
...
structure DedekindKunnethShortExact
    ...
    [HasDerivedCategory.{w} (Mod R)]
    (K L : Formalization.Books.MoreAlgebra.Unit56.DBounded R) (n : ℤ) where
  sequence : ShortComplex (Mod R)
  exact : sequence.ShortExact
  left_iso : Nonempty (sequence.X₁ ≅
    ∐ fun ij : {x : ℤ × ℤ // x.1 + x.2 = n} =>
      MonoidalCategory.tensorObj
        (boundedDerivedCohomology K ij.1.1 : Mod R)
        (boundedDerivedCohomology L ij.1.2 : Mod R))
...
theorem kunneth_dedekind_shortExact
    {R : Type u} [CommRing R] [IsDedekindDomain R]
    [HasDerivedCategory.{w} (Mod R)]
    (K L : Formalization.Books.MoreAlgebra.Unit56.DBounded R) (n : ℤ) :
