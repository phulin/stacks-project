...
private theorem pointedAlgebraicStructureType :
    AlgebraicStructureType (Pointed.{u}) (forget Pointed) := by
...
private theorem standardNonLieAlgebraicStructureTypes :
    AlgebraicStructureType (AddCommGrpCat.{u}) (forget AddCommGrpCat) ∧
      ...
      (∀ (R : Type r) [Ring R],
        AlgebraicStructureType (ModuleCat.{u} R) (forget (ModuleCat.{u} R))) := by
...
private theorem lieLimitInstances (K : Type k) [Field K] :
    HasLimitsOfSize.{u, u} (LieAlgebraCat.{u} K) ∧
  ...
  let : PreservesLimitsOfSize.{u, u} (forget (LieAlgebraCat.{u} K)) :=
    liePreserves
  exact ⟨lieLimits, liePreserves⟩

private theorem lieFilteredColimitInstances (K : Type k) [Field K] :
    HasFilteredColimitsOfSize.{u, u} (LieAlgebraCat.{u} K) ∧
  ...
  exact ⟨lieFiltered, liePreservesFiltered⟩

private theorem lieReflectsIso (K : Type k) [Field K] :
...
private theorem lieAlgebraicStructureType (K : Type k) [Field K] :
    AlgebraicStructureType (LieAlgebraCat.{u} K)
      (forget (LieAlgebraCat.{u} K)) := by
  obtain ⟨lieLimits, liePreserves⟩ := lieLimitInstances K
  obtain ⟨lieFiltered, liePreservesFiltered⟩ := lieFilteredColimitInstances K
  let : HasLimitsOfSize.{u, u} (LieAlgebraCat.{u} K) := lieLimits
  let : PreservesLimitsOfSize.{u, u} (forget (LieAlgebraCat.{u} K)) := liePreserves
  let : HasFilteredColimitsOfSize.{u, u} (LieAlgebraCat.{u} K) := lieFiltered
  let : PreservesFilteredColimitsOfSize.{u, u}
      (forget (LieAlgebraCat.{u} K)) := liePreservesFiltered
  let : (forget (LieAlgebraCat.{u} K)).ReflectsIsomorphisms := lieReflectsIso K
  exact ⟨⟩
...
theorem standardAlgebraicStructureTypes :
  ...
  obtain ⟨hAdd, hGrp, hMon, hRing, hModule⟩ :=
    standardNonLieAlgebraicStructureTypes
  exact ⟨pointedAlgebraicStructureType, hAdd, hGrp, hMon, hRing,
    hModule, fun K _ => lieAlgebraicStructureType K⟩
