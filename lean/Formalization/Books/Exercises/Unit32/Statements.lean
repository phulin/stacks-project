...
theorem sheaf_mono_iff_stalk_injective
    {X : TopCat.{v}} {F G : SetSheaves X} (φ : F ⟶ G) :
    Mono φ ↔ ∀ x : X, Function.Injective (setStalkMap φ x) := by
...
theorem sheaf_isIso_iff_stalk_bijective
    {X : TopCat.{v}} {F G : SetSheaves X} (φ : F ⟶ G) :
    IsIso φ ↔ ∀ x : X, Function.Bijective (setStalkMap φ x) := by
...
theorem sheaf_epi_iff_stalk_surjective
    {X : TopCat.{v}} {F G : SetSheaves X} (φ : F ⟶ G) :
    Epi φ ↔ ∀ x : X, Function.Surjective (setStalkMap φ x) := by
...
theorem extensionByEmpty_stalk_isEmpty
    {X : TopCat.{v}} (U : Opens X)
    (G : SetSheaves (openSubspace U)) (x : X) (hx : x ∉ U) :
    IsEmpty (((extensionByEmpty U).obj G).presheaf.stalk x) := by
  exact openSetSheafExtension_stalk_empty U G x hx
...
theorem extensionByEmpty_stalk_iso
  ...
  exact openSetSheafExtension_stalk_iso U G x hx
...
theorem extensionByZero_stalk_zero
    {X : TopCat.{v}} (U : Opens X)
    (G : AbelianSheaves (openSubspace U)) (x : X) (hx : x ∉ U) :
    Nonempty (((extensionByZero U).obj G).presheaf.stalk x ≅
      (⊥_ AddCommGrpCat.{v})) := by
  exact openAlgebraicSheafExtension_stalk_initial AddCommGrpCat U G x hx
...
theorem extensionByZero_stalk_iso
    ...
    Nonempty (((extensionByZero U).obj G).presheaf.stalk x ≅
      G.presheaf.stalk ⟨x, hx⟩) := by
  exact openAlgebraicSheafExtension_stalk_iso AddCommGrpCat U G x hx
...
theorem extensionByEmpty_and_zero_are_distinct_constructions
    {X : TopCat.{v}} (U : Opens X)
    (G : SetSheaves (openSubspace U)) (H : AbelianSheaves (openSubspace U))
    (x : X) (hx : x ∉ U) :
...
theorem realIdealSheaf_not_locally_generated :
    ¬ Formalization.Books.Modules.Unit08.additiveLocallyGenerated
      realIdealSheaf := by
  sorry
...
theorem every_abelian_sheaf_is_quotient_of_integer_extensions
    {X : TopCat.{v}} (F : AbelianSheaves X) :
    ∃ (I : Type v) (G : I → AbelianSheaves X)
...
theorem directSumSheafOfAbelianSheaves_stalk_iso
    {X : TopCat.{v}} {I : Type v} (F : I → AbelianSheaves X) (x : X) :
...
theorem modifiedProduct_sections_need_not_equal_basis_subgroup :
    ∃ (X : TopCat) (A : X → AddCommGrpCat) (D : ModifiedProductData A)
      ...
      (modifiedProductSetSheaf D).presheaf.obj (op U) ≠
        D.subgroup U hU := by
  sorry
