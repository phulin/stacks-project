...
/-- A linear equivalence identifies the annihilator ideals of corresponding
elements, hence also their sets of minimal primes. -/
theorem mem_minimalPrimes_colon_linearEquiv
    {A X Y : Type*} [CommRing A] [AddCommGroup X] [AddCommGroup Y]
    [Module A X] [Module A Y] (e : X ≃ₗ[A] Y) (p : PrimeSpectrum A) (x : X) :
    p.asIdeal ∈
        ((⊥ : Submodule A Y).colon ({e x} : Set Y)).minimalPrimes ↔
      p.asIdeal ∈
        ((⊥ : Submodule A X).colon ({x} : Set X)).minimalPrimes := by
  have hcolon :
      ((⊥ : Submodule A X).colon ({x} : Set X)) =
        ((⊥ : Submodule A Y).colon ({e x} : Set Y)) := by
    ext a
...
/-- A finitely generated field extension is finite over the field generated by
a finite transcendence basis. -/
theorem exists_finite_transcendenceBasis_of_fg
    {k K : Type*} [Field k] [Field K] [Algebra k K]
    (hfg : (⊤ : IntermediateField k K).FG) :
    ∃ s : Set K, s.Finite ∧ IsTranscendenceBasis k ((↑) : s → K) ∧
      Module.Finite (IntermediateField.adjoin k s) K := by
  classical
  obtain ⟨u, hu⟩ := hfg
  obtain ⟨s, hs⟩ := exists_isTranscendenceBasis k K
  let F : IntermediateField k K := IntermediateField.adjoin k (s : Set K)
  haveI hAlg : Algebra.IsAlgebraic F K := by
    have h := hs.isAlgebraic_field
    rw [show Set.range ((↑) : s → K) = s by exact Subtype.range_val] at h
    exact h
  have htop : IntermediateField.adjoin F (u : Set K) = ⊤ := by
    apply IntermediateField.restrictScalars_injective k
    rw [IntermediateField.restrictScalars_adjoin, IntermediateField.restrictScalars_top]
    apply top_unique
    rw [← hu]
    rw [IntermediateField.adjoin_le_iff]
    intro x hx
    exact IntermediateField.subset_adjoin k _ (Or.inr hx)
  haveI hess : Algebra.EssFiniteType F K :=
    IntermediateField.fg_top_iff.mp ⟨u, htop⟩
  have hmodule : Module.Finite F K :=
    Algebra.finite_of_essFiniteType_of_isAlgebraic
  have htrdeg : Algebra.trdeg k K ≤ Cardinal.mk u := by
    let Fu : IntermediateField k K := IntermediateField.adjoin k (u : Set K)
    letI : Algebra.IsAlgebraic Fu K := by
      constructor
      intro x
      have hx : x ∈ Fu := by
        change x ∈ IntermediateField.adjoin k (u : Set K)
        rw [hu]
        trivial
      let y : Fu := ⟨x, hx⟩
      have hy : IsAlgebraic Fu (algebraMap Fu K y) := isAlgebraic_algebraMap y
      have heq : algebraMap Fu K y = x := rfl
      rw [heq] at hy
      exact hy
    letI : Algebra.IsAlgebraic (Algebra.adjoin k (u : Set K)) K :=
      Algebra.IsAlgebraic.trans _ Fu _
    exact Algebra.IsAlgebraic.trdeg_le_cardinalMk k (u : Set K)
...
theorem exists_weaklyAssociatedPrime_over_finite_intermediateField
    {k K R N : Type*} [Field k] [Field K] [Algebra k K]
    [CommRing R] [Algebra k R] (L : IntermediateField k K)
    [Module.Finite L K] [AddCommGroup N] [Module (R ⊗[k] K) N]
...
theorem weaklyAssociatedPrimes_descend_finite_intermediateField_module
...
/- The original monolithic proof shell is retained here as historical context.
The compiled theorem now lives in `ChangeFields.lean`, where it can depend on
the focused localization and finite-type reduction files without an import
cycle.
...
theorem weaklyAssociatedPrimes_change_fields
  ...
  proof moved to `ChangeFields.lean`
-/
end
