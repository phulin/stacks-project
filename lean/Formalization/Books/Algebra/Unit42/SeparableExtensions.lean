...
/- A finite purely inseparable extension can require several successive
   p-th-root steps.  The level parameter records the resulting p^n-th root
   without changing the established one-step construction above. -/
noncomputable def pthRootInAlgebraicClosureAtLevel
    (F : Type u) [Field F] (p : ℕ) (hp : 0 < p) (n : ℕ) (a : F) :
    AlgebraicClosure F :=
  Classical.choose <|
    IsAlgClosed.exists_pow_nat_eq (algebraMap F (AlgebraicClosure F) a)
      (n := p ^ n) (Nat.pow_pos hp)

@[simp]
theorem pthRootInAlgebraicClosureAtLevel_pow
    (F : Type u) [Field F] (p : ℕ) (hp : 0 < p) (n : ℕ) (a : F) :
    pthRootInAlgebraicClosureAtLevel F p hp n a ^ (p ^ n) =
      algebraMap F (AlgebraicClosure F) a :=
  Classical.choose_spec <|
    IsAlgClosed.exists_pow_nat_eq (algebraMap F (AlgebraicClosure F) a)
      (n := p ^ n) (Nat.pow_pos hp)

/- The finite field generated by finitely many roots at an arbitrary finite
   purely inseparable level. -/
noncomputable def finitePthRootFieldAtLevel
    (F : Type u) [Field F] (p : ℕ) (hp : 0 < p) (n : ℕ) (s : Finset F) :
    IntermediateField F (AlgebraicClosure F) :=
  IntermediateField.adjoin F
    (range fun a : s => pthRootInAlgebraicClosureAtLevel F p hp n (a : F))

theorem finitePthRootFieldAtLevel_finiteDimensional
    (F : Type u) [Field F] (p : ℕ) (hp : 0 < p) (n : ℕ) (s : Finset F) :
    FiniteDimensional F (finitePthRootFieldAtLevel F p hp n s) := by
...
theorem finitePthRootFieldAtLevel_isPurelyInseparable
  ...
  rintro _ ⟨a, rfl⟩
  refine ⟨n, ?_⟩
  rw [pthRootInAlgebraicClosureAtLevel_pow F p hp n (a : F)]
  exact ⟨a, rfl⟩
...
/- The corresponding top field at an arbitrary finite p-th-root level. -/
noncomputable def finitePthRootTopAtLevel
    (k : Type u) (K : Type v) [Field k] [Field K] [Algebra k K]
    (p : ℕ) (hp : 0 < p) (n : ℕ) (s : Finset k) (t : Finset K) :
    IntermediateField K (AlgebraicClosure K) :=
  IntermediateField.adjoin K
    ((range fun a : s =>
      pthRootClosureMap k K
        (pthRootInAlgebraicClosureAtLevel k p hp n (a : k))) ∪
      (range fun b : t =>
        pthRootInAlgebraicClosureAtLevel K p hp n (b : K)))

theorem finitePthRootTopAtLevel_finiteDimensional
    (k : Type u) (K : Type v) [Field k] [Field K] [Algebra k K]
    (p : ℕ) (hp : 0 < p) (n : ℕ) (s : Finset k) (t : Finset K) :
    FiniteDimensional K (finitePthRootTopAtLevel k K p hp n s t) := by
  apply IntermediateField.finiteDimensional_adjoin
  intro x hx
  exact Algebra.IsIntegral.isIntegral x
...
theorem finitePthRootTopAtLevel_isPurelyInseparable
  ...
  · refine ⟨n, ?_⟩
    change (pthRootClosureMap k K
      (pthRootInAlgebraicClosureAtLevel k p hp n (a : k))) ^ (p ^ n) ∈
      (algebraMap K (AlgebraicClosure K)).range
    rw [← map_pow, pthRootInAlgebraicClosureAtLevel_pow k p hp n (a : k)]
    exact ⟨algebraMap k K a, by
      rw [← IsScalarTower.algebraMap_apply k K (AlgebraicClosure K) (a : k)]
      exact ((pthRootClosureMap k K).commutes a).symm⟩
  · refine ⟨n, ?_⟩
    change (pthRootInAlgebraicClosureAtLevel K p hp n (b : K)) ^ (p ^ n) ∈
      (algebraMap K (AlgebraicClosure K)).range
    rw [pthRootInAlgebraicClosureAtLevel_pow K p hp n (b : K)]
    exact ⟨b, rfl⟩
...
/- The level-indexed construction has the same base-to-top map. -/
theorem finitePthRootFieldAtLevel_map_mem_top
    (k : Type u) (K : Type v) [Field k] [Field K] [Algebra k K]
    (p : ℕ) (hp : 0 < p) (n : ℕ) (s : Finset k) (t : Finset K)
    (x : finitePthRootFieldAtLevel k p hp n s) :
    pthRootClosureMap k K x ∈ finitePthRootTopAtLevel k K p hp n s t := by
  have hle :
      (finitePthRootFieldAtLevel k p hp n s).map (pthRootClosureMap k K) ≤
        (finitePthRootTopAtLevel k K p hp n s t).restrictScalars k := by
    rw [finitePthRootFieldAtLevel, IntermediateField.adjoin_map]
    rw [IntermediateField.adjoin_le_iff]
    rintro _ ⟨a, ⟨a₀, rfl⟩, rfl⟩
    change _ ∈ finitePthRootTopAtLevel k K p hp n s t
    exact IntermediateField.subset_adjoin K _ (Or.inl ⟨a₀, rfl⟩)
  exact hle ((IntermediateField.map_mem_map
    (S := finitePthRootFieldAtLevel k p hp n s) (pthRootClosureMap k K)).2
      x.property)

noncomputable def finitePthRootBaseToTopAtLevel
    (k : Type u) (K : Type v) [Field k] [Field K] [Algebra k K]
    (p : ℕ) (hp : 0 < p) (n : ℕ) (s : Finset k) (t : Finset K) :
    finitePthRootFieldAtLevel k p hp n s →+*
      finitePthRootTopAtLevel k K p hp n s t :=
  RingHom.codRestrict (pthRootClosureMap k K |>.comp
    (finitePthRootFieldAtLevel k p hp n s).val)
    (finitePthRootTopAtLevel k K p hp n s t)
    (fun x => finitePthRootFieldAtLevel_map_mem_top k K p hp n s t x)

noncomputable instance finitePthRootBaseAlgebraAtLevel
    (k : Type u) (K : Type v) [Field k] [Field K] [Algebra k K]
    (p : ℕ) (hp : 0 < p) (n : ℕ) (s : Finset k) (t : Finset K) :
    Algebra (finitePthRootFieldAtLevel k p hp n s)
      (finitePthRootTopAtLevel k K p hp n s t) :=
  RingHom.toAlgebra (finitePthRootBaseToTopAtLevel k K p hp n s t)

noncomputable instance finitePthRootBaseTowerAtLevel
    (k : Type u) (K : Type v) [Field k] [Field K] [Algebra k K]
    (p : ℕ) (hp : 0 < p) (n : ℕ) (s : Finset k) (t : Finset K) :
    IsScalarTower k (finitePthRootFieldAtLevel k p hp n s)
      (finitePthRootTopAtLevel k K p hp n s t) := by
  apply IsScalarTower.of_algebraMap_eq'
  ext a
  change algebraMap k (AlgebraicClosure K) a =
    pthRootClosureMap k K (algebraMap k (AlgebraicClosure k) a)
...
noncomputable def finitePthRootBaseChangeAtLevel
    ...
    (hsep : IsSeparablyGenerated
      ...
      (finitePthRootTopAtLevel k K p hp n s t)) :
    PurelyInseparableBaseChange k K := by
  let B := finitePthRootFieldAtLevel k p hp n s
  let T := finitePthRootTopAtLevel k K p hp n s t
  letI : Algebra B T := finitePthRootBaseAlgebraAtLevel k K p hp n s t
  letI : IsScalarTower k B T := finitePthRootBaseTowerAtLevel k K p hp n s t
  letI : FiniteDimensional k B :=
    finitePthRootFieldAtLevel_finiteDimensional k p hp n s
  letI : IsPurelyInseparable k B :=
    finitePthRootFieldAtLevel_isPurelyInseparable k p hp n s
  letI : FiniteDimensional K T :=
    finitePthRootTopAtLevel_finiteDimensional k K p hp n s t
  letI : IsPurelyInseparable K T :=
    finitePthRootTopAtLevel_isPurelyInseparable k K p hp n s t
  exact { base := B, top := T, topSeparablyGenerated := hsep }
...
theorem exists_purely_inseparable_base_change
  ...
  · obtain _ | ⟨p, hp, hpk⟩ := CharP.exists' k
    ...
    · let _ : Fact p.Prime := hp
      ...
      exact ⟨finitePthRootBaseChangeAtLevel k K p hp.out.pos n s t hsep⟩
