...
theorem IsGlobalCompleteIntersection.finiteType
    {k S : Type u} [Field k] [CommRing S] [Algebra k S]
    (h : IsGlobalCompleteIntersection k S) :
    RingHom.FiniteType (algebraMap k S) :=
  h.1

theorem IsLocalCompleteIntersection.finiteType
    {k S : Type u} [Field k] [CommRing S] [Algebra k S]
    (h : IsLocalCompleteIntersection k S) :
    RingHom.FiniteType (algebraMap k S) :=
  h.1
...
/-- At an lci presentation prime, generation of the localized defining ideal
and generation of its conormal module require the same number of elements. -/
theorem presentation_idealGenerated_iff_conormalGenerated
    {k S : Type u} [Field k] [CommRing S] [Algebra k S]
    {n : ℕ} (P : PolynomialPresentation k S n)
    (q : PrimeSpectrum S) (q' : PrimeSpectrum P.Ring)
    (hcorresponding : PrimeSpectrum.comap (algebraMap P.Ring S) q = q')
    (c : ℕ) (hc : q'.asIdeal.height = q.asIdeal.height + (c : ℕ∞)) :
    IsIdealGeneratedByElements (LocalizedPresentationIdeal P q') c ↔
      IsModuleGeneratedByElements (LocalizedPresentationQuotient P q')
        (LocalizedPresentationConormal P q') c := by
  exact (local_complete_intersection_at_presentation_prime
    P q q' hcorresponding c hc).out 1 2

/-- At an lci presentation prime, the localized conormal module is free of
the expected rank exactly when the localized kernel is generated by a regular
sequence. -/
theorem presentation_conormalFree_iff_kernelRegularSequence
    {k S : Type u} [Field k] [CommRing S] [Algebra k S]
    {n : ℕ} (P : PolynomialPresentation k S n)
    (q : PrimeSpectrum S) (q' : PrimeSpectrum P.Ring)
    (hcorresponding : PrimeSpectrum.comap (algebraMap P.Ring S) q = q')
    ...
    IsFreeOfRank (LocalizedPresentationQuotient P q')
        (LocalizedPresentationConormal P q') c ↔
      IsIdealGeneratedByRegularSequence
        (LocalizedPresentationIdeal P q') := by
  exact (local_complete_intersection_at_presentation_prime
    P q q' hcorresponding c hc).out 3 4
...
/-- For a surjection from a regular local ring, the regular-sequence and
dimension-difference formulations of complete intersection agree. -/
theorem regularKernel_iff_dimensionKernel
    {B C : Type u} [CommRing B] [CommRing C] [IsLocalRing C]
    [IsRegularLocalRing B] (g : B →+* C) [IsLocalHom g]
    (hg : Function.Surjective g) :
    IsIdealGeneratedByRegularSequence (RingHom.ker g) ↔
      IsGeneratedByDimensionDifference B C (RingHom.ker g) := by
  have h := complete_intersection_well_defined
    (RingHom.id B) g (Function.surjective_id) hg
  exact h.out 2 3
