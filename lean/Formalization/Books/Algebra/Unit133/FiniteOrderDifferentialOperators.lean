...
theorem isDifferentialOperator_smul (k : ℕ) (c : S) (D : LinearMapOver R M N)
  ...
  | succ k ih =>
      ...
      rw [show differentialOperatorCommutator (c • D) s =
        ...
        simp [differentialOperatorCommutator, LinearMap.smul_comp,
          smul_sub, hcomp]]
      exact ih _ (hD s)
...
omit R [CommRing R] [Algebra R S] [Module R M] [IsScalarTower R S M] in
private theorem principalPartsHigherRelation_succ_eq (k : ℕ)
  ...
  have hfalse : (∑ x : Finset (Fin (k + 1)), F (embed (x, false))) =
    ...
    simp [F, embed, principalPartsHigherRelation, g₀, a, hcompFalse,
      Finset.smul_sum, mul_comm, mul_left_comm]
...
private theorem principalPartsHigherEvaluation_eq_map
  ...
  simp [principalPartsHigherEvaluation, principalPartsHigherEvaluationMap,
    principalPartsHigherRelation, Finsupp.lsum_apply, smul_smul, mul_comm]
...
private theorem principalPartsHigherEvaluation_succ
  ...
  rw [principalPartsHigherEvaluation_eq_map,
    principalPartsHigherRelation_succ_eq (S := S) (M := M)]
  ...
  | succ k ih =>
      ...
      have hcomm :
        ...
        simp [principalPartsHigherEvaluation, differentialOperatorCommutator,
          Finset.smul_sum, smul_sub, smul_smul, mul_comm, mul_left_comm]
...
theorem principalParts_universal_linear_map_exists (k : ℕ) :
  ...
  have hq : principalPartsHigherEvaluationMap (R := R) (S := S) u = q := by
    ext z
    classical
    simp [principalPartsHigherEvaluationMap, u, q]
  refine ⟨u, ?_, ?_⟩
  · intro m
    rfl
...
private theorem principalParts_factorization_unique_aux (k : ℕ) (N : Type*)
    ...
    · rcases hx with ⟨p, rfl⟩ | ⟨⟨r, m⟩, rfl⟩
      · change L (principalPartsAddRelation p.1 p.2) = 0
        ...
        rw [map_sub, map_sub, Finsupp.lsum_single, Finsupp.lsum_single,
          Finsupp.lsum_single]
        simp [D.1.map_add, sub_eq_add_neg, add_assoc]
  ...
  have hex : ∃ α : PrincipalParts (R := R) (S := S) (M := M) k →ₗ[S] N,
    ...
    change L (Finsupp.single m 1) = D.1 m
    simp [L, Finsupp.lsum_single]
  ...
  let σ : Derivation R S N :=
    { toLinearMap := D.1 -
      ...
      leibniz' := by
        ...
        rw [← hc'']
        simp [smul_smul, sub_eq_add_neg, add_assoc, add_comm, add_left_comm] }
  refine ⟨σ, ?_⟩
  constructor
  · intro g
    rfl
  ...
  let backward :
    ...
    { toFun := fun p => ⟨op p, hop p⟩
      map_add' := by
        ...
        apply Subtype.ext
        ext g
        simp [op, scalarValueLinearMap, add_assoc, add_comm, add_left_comm]
      map_smul' := by
        intro c p
        apply Subtype.ext
        ext g
        simp [op, scalarValueLinearMap, smul_smul, mul_comm] }
  have hback :
      ∀ D : differentialOperatorSubmodule (R := R) (S := S) (M := S) (N := N) 1,
...
theorem principalParts_one_equiv_differentials_prod :
  ...
  have hdprod :
    ...
    have hcomm :
      ...
      apply Prod.ext
      · simp [Derivation.leibniz, add_assoc, sub_eq_add_neg]
      · simp
    rw [hcomm]
    intro s m
...
private theorem principalPartsHigherRelation_one_explicit
  ...
  rw [hd0, hd1, hd2, hd3]
  simp only [Finset.card_empty, Finset.card_singleton,
    Finset.prod_empty, Finset.prod_singleton,
    pow_zero, pow_one, one_smul, neg_one_smul, Finsupp.smul_single',
    sub_eq_add_neg]
  simp [pow_two, mul_one]
  abel
...
theorem principalParts_functoriality_exists
  ...
  have hqhigher (k : ℕ) (g : Fin (k + 1) → B) (m : M) :
    ...
    intro t ht
    ext i
    simp [q, Finsupp.mapDomain, Finsupp.mapRange, hf]
...
The former interface instead fixed carriers `B' : Type uB'` and
`M' : Type uM'` in independent universes.  Choosing the intended carriers
then generated the unsolvable equations `uB' = max uB uA'` and
`uM' = max uM uA'`; even at matching levels, arbitrary fixed carriers supply
no equivalences with the tensor products.  Thus the base-changed carriers and
their non-global instances have to be output data, as they are here.
-/
...
Use the following names throughout (all have the displayed literal carrier,
not a universe lift):

* `B₁ : Type (max uB uA') := B ⊗[A] A'`;
* `M₁ : Type (max uM uA') := M ⊗[A] A'`;
* `P := PrincipalParts (R := A) (S := B) (M := M) k`;
* `Q := P ⊗[A] A'`; and
* `P₁ := PrincipalParts (R := A') (S := B₁) (M := M₁) k`.

The first two are the `B'` and `M'` fields, so `identify_B'` and
`identify_M'` are `RingEquiv.refl B₁` and `LinearEquiv.refl A M₁`.  Do not
use `(B ⊗[A] A') ⊗[B] M` for `M₁`: its universe also contains `uB`, which
recreates the failed representative problem.
...
1. Install the canonical structures without changing these two types.
   ...
   `X = M` and `X = P`, let
   `cX := TensorProduct.comm A X A' : X ⊗[A] A' ≃ₗ[A] A' ⊗[A] X` and install
   `cX.toAddEquiv.module A'` on its source; the resulting
   `cX.toAddEquiv.linearEquiv A'` is the `A'`-linear commutor.  Install the
   accompanying `IsScalarTower A A' (X ⊗[A] A')` with
   `LinearEquiv.isScalarTower`.  The left `B`-module and its `A`-tower come
   from `TensorProduct.leftModule` and `TensorProduct.isScalarTower_left`.
   Prove `SMulCommClass B A' (X ⊗[A] A')` by
   `TensorProduct.induction_on`; on `x ⊗ₜ a'`, unfold the transported action
   with `TensorProduct.comm_tmul`.  Now set the `B₁`-module to
   `TensorProduct.Algebra.module`; its pure-scalar action is
   `TensorProduct.Algebra.smul_def`.  Finally construct
   `IsScalarTower A' B₁ (X ⊗[A] A')` with
   `IsScalarTower.of_algebraMap_smul`, reducing to that pure-scalar rule and
   `Algebra.TensorProduct.right_algebraMap_apply`.  Retain the `X = M` tower
   and both `B₁`-modules as structure fields.  The transport declarations are
   in `Mathlib/Algebra/Module/TransferInstance.lean`;
   `TensorProduct.leftModule` and `TensorProduct.isScalarTower_left` are in
   `Mathlib/LinearAlgebra/TensorProduct/Defs.lean`; the commutor is in
   `Mathlib/LinearAlgebra/TensorProduct/Basic.lean`; and the tensor-algebra
   declarations are in `Mathlib/RingTheory/TensorProduct/Basic.lean`.

2. Put `f m := m ⊗ₜ[A] 1`.  Its `B`-semilinearity follows from
   `TensorProduct.Algebra.smul_def`, while the square of algebra maps is
   `Algebra.TensorProduct.includeLeftRingHom_comp_algebraMap`, used symmetrically
   because the goal lists the right inclusion first.  Apply
   `principalParts_functoriality_exists` (earlier in this file) and choose
   `F`.  Package `F.map k` as `Fₖ : P →ₗ[A] P₁`; prove `map_smul'` by rewriting
   `a • p` through `algebraMap A B`, applying `F.map_smul`, and then the
   algebra-map square.  Let `L : Q →ₗ[A'] P₁` be
   `(Fₖ.liftBaseChange A').comp cP`, where `cP : Q ≃ₗ[A'] A' ⊗[A] P` is the
   transported commutor from Step 1.  Promote the same function to
   `Φ : Q →ₗ[B₁] P₁`: prove its scalar law first for a scalar
   `b ⊗ₜ[A] a'` and an argument `p ⊗ₜ[A] c'`, using `F.map_smul`, then induct
   on the argument and scalar with `TensorProduct.induction_on`.
   `LinearMap.liftBaseChange` and `LinearMap.liftBaseChange_tmul` are in
   `Mathlib/RingTheory/TensorProduct/Basic.lean`.

3. Let `U := principalPartsUniversalLinearMap (R := A) (S := B)
   (M := M) k : M →ₗ[A] P`.  Define `D : M₁ →ₗ[A'] Q` as
   `cP.symm.comp ((U.baseChange A').comp cM)`, where
   `cM : M₁ ≃ₗ[A'] A' ⊗[A] M` and
   `cP : Q ≃ₗ[A'] A' ⊗[A] P` are from Step 1.  From
   `LinearMap.baseChange_tmul` and `TensorProduct.comm_tmul` obtain the named
   rule
   `hD_tmul : D (m ⊗ₜ[A] a') =
     principalPartsGenerator (R := A) (S := B) k m ⊗ₜ[A] a'`.
   Here `LinearMap.baseChange_tmul` is in
   `Mathlib/LinearAlgebra/TensorProduct/Tower.lean`.

   Before using the universal property, prove a chronological local helper:
   for every `j` and every `E : M →ₗ[A] P` satisfying
   `IsDifferentialOperator (R := A) (S := B) j E`, its commuted base change
   `E' : M₁ →ₗ[A'] Q` satisfies
   `IsDifferentialOperator (R := A') (S := B₁) j E'`.  Induct on `j`; in the
   successor case induct on a coefficient of `B₁`.  Zero and addition reduce
   with `isDifferentialOperator_zero` and `isDifferentialOperator_add`.  For
   `b ⊗ₜ[A] a'`, prove by extensionality and tensor induction that
   `differentialOperatorCommutator E' (b ⊗ₜ a')` is
   `(1 ⊗ₜ[A] a') •` the commuted base change of
   `differentialOperatorCommutator E b`, then apply the order induction
   hypothesis and `isDifferentialOperator_smul`.  Instantiate this helper at
   `U` using `principalPartsUniversalLinearMap_isDifferentialOperator` (both
   declarations are earlier in this file).

4. Feed the resulting differential operator to
   `principalPartsHomEquiv (R := A') (S := B₁) (M := M₁) k Q`; explicitly
   package `D₀ : differentialOperatorSubmodule (R := A') (S := B₁)
   (M := M₁) (N := Q) k := ⟨D, hD⟩` and set the resulting map to
   `Ψ : P₁ →ₗ[B₁] Q`.  `principalPartsHomEquiv_factorization`,
   `principalPartsUniversalLinearMap_apply`, and `hD_tmul` give
   `Ψ (principalPartsGenerator k (m ⊗ₜ a')) =
     principalPartsGenerator k m ⊗ₜ a'`.
   These three principal-parts declarations are earlier in
   `Formalization/Books/Algebra/Unit133/FiniteOrderDifferentialOperators.lean`.

5. For `Φ.comp Ψ = LinearMap.id`, use
   `principalParts_factorization_unique` (earlier in this file), checking the
   two factorizations on the universal map for `P₁`; reduce an arbitrary
   `m' : M₁` to pure tensors with `TensorProduct.induction_on` and use the
   generator formula from Step 4.  For the other composite first prove the
   named claim `hΨF (p : P) : Ψ (F.map k p) = p ⊗ₜ[A] 1`: choose a
   `Finsupp` representative with `Submodule.mkQ_surjective`, use
   `Finsupp.induction_linear`, and in the single case use `F.map_generator`,
   `F.map_smul`, and the Step 4 formula.  A final tensor induction, using
   `LinearMap.liftBaseChange_tmul`, gives
   `Ψ.comp Φ = LinearMap.id`.  Assemble `equivalence` as
   `LinearEquiv.ofLinear Φ Ψ` in the order
   `(Φ.comp Ψ = id), (Ψ.comp Φ = id)`, then fill the structure with the
   instances and reflexive identifications from Step 1.
...
theorem principalParts_diagonal_equiv_exists (k : ℕ) :
  ...
  have hD : IsDifferentialOperator (R := R) (S := S) k D := by
    ...
    simp only [principalPartsHigherEvaluationMap, principalPartsHigherRelation,
      Finsupp.lsum_apply, Finsupp.smul_single', mul_comm, mul_left_comm]
  ...
  have hβgen (m : M) :
    ...
    change L ((TensorProduct.AlgebraTensorModule.mk R S S M 1) m) = U m
    simp [L, φ]
  have hβα : β.comp α = LinearMap.id := by
    ...
    induction F using Finsupp.induction_linear with
    | zero => simp
    | add F G hF hG => simp only [map_add, LinearMap.id_apply, hF, hG]
    | single m c =>
        calc
  ...
  have hαβ : α.comp β = LinearMap.id := by
    ...
    · intro s m
      ...
      simp [TensorProduct.smul_tmul']
    · intro x y hx hy
      simp only [map_add, LinearMap.id_apply, hx, hy]
...
theorem deRham_differential_is_differentialOperator (i : ℕ) :
        ...
        { toFun := fun q => exteriorPower.ιMulti B (i + 1) (Fin.cons q w)
          map_add' := by
            ...
            simp [exteriorPower.ιMulti, ExteriorAlgebra.ιMulti_succ_apply,
              Matrix.vecTail, add_mul]
          map_smul' := by
            intro c q
            apply Subtype.ext
            simp [exteriorPower.ιMulti, ExteriorAlgebra.ιMulti_succ_apply,
              Matrix.vecTail, Algebra.smul_def, mul_assoc] }
      have hW (q : B) :
  ...
  · rintro _ ⟨z, rfl⟩
    rcases z with ⟨b₀, b⟩
    simpa [smul_smul, mul_comm] using hgen b₀ b
  · simp
...
theorem differentialOperator_check_on_algebra_generators
  ...
  have hcomm_mul (x y : B) :
    ...
    ext m
    simp [differentialOperatorCommutator, DistribSMul.toLinearMap,
      sub_eq_add_neg, add_assoc, add_left_comm, mul_smul, smul_add]
...
theorem differentialOperator_localization_unique (T : Submonoid B) (k : ℕ)
  ...
  | succ k ih =>
            ...
            calc
              ...
              _ = (c : B) • LocalizedModule.mkLinearMap T N (D.1 m) +
                ...
                change LocalizedModule.mk ((c : B) • D.1 m) 1 +
                  ...
                  (c : B) •
                      (LocalizedModule.mk (S := T) (D.1 m) (1 : T) :
                        LocalizedModule T N) +
                    LocalizedModule.mk (S := T) ((Dcomm (c : B)).1 m) (1 : T)
      ...
      have hraw_wd : ∀ p p' : M × T, p ≈ p' → raw p = raw p' := by
        rintro ⟨m, s⟩ ⟨m', s'⟩ ⟨c, hc⟩
        calc
          raw (m, s) = raw (((c * s' : T) : B) • m, (c * s') * s) :=
            hraw_cancel (c * s') s m
          _ = raw (((c * s : T) : B) • m', (c * s) * s') := by
            congr 1
            · simpa [Submonoid.smul_def, smul_smul, mul_assoc, mul_comm,
                mul_left_comm] using hc
          _ = raw (m', s') := (hraw_cancel (c * s) s' m').symm
      let Efun : LocalizedModule T M → LocalizedModule T N := fun x =>
        x.liftOn raw hraw_wd
      have hEfun_mk (m : M) (s : T) :
          Efun (LocalizedModule.mk m s) = raw (m, s) := by
        change (LocalizedModule.mk m s).liftOn raw hraw_wd = raw (m, s)
        exact LocalizedModule.liftOn_mk hraw_wd m s
      have hmk_add (a b : M) (s : T) :
          LocalizedModule.mk (a + b) s =
            LocalizedModule.mk a s + LocalizedModule.mk b s := by
        rw [LocalizedModule.mk_add_mk]
        apply LocalizedModule.mk_eq.mpr
        refine ⟨1, ?_⟩
        simp [smul_add, mul_smul]
      have hmk_addN (a b : N) (s : T) :
          LocalizedModule.mk a s + LocalizedModule.mk b s =
            LocalizedModule.mk (a + b) s := by
        symm
        rw [LocalizedModule.mk_add_mk]
        apply LocalizedModule.mk_eq.mpr
        refine ⟨1, ?_⟩
        simp [smul_add, mul_smul]
      have hEfun_add (x y : LocalizedModule T M) :
          Efun (x + y) = Efun x + Efun y := by
        induction x, y using LocalizedModule.induction_on₂ with
        | _ m m' s s' =>
            rw [LocalizedModule.mk_add_mk, hEfun_mk, hEfun_mk, hEfun_mk]
            rw [hraw_cancel s' s m, hraw_cancel s s' m']
            simp only [raw]
            rw [D.1.map_add]
            rw [(LocalizedModule.mkLinearMap T N).map_add]
            rw [show s' * s = s * s' by ac_rfl]
            rw [← (LocalizedModule.divBy (s * s')).map_add]
            congr 1
            rw [hprod (s : B) (s' : B)]
            simp only [LinearMap.add_apply, LinearMap.comp_apply]
            rw [hmk_add (s' • m) (s • m') (s * s')]
            simp only [map_add, mulM, mulN, DistribSMul.toLinearMap_apply]
            simp only [LocalizedModule.mkLinearMap_apply]
            have hcancel1 :
                LocalizedModule.mk ((s' : B) • m) (s * s') =
                  LocalizedModule.mk m s := by
              simpa only [Submonoid.smul_def, mul_comm] using
                (LocalizedModule.mk_cancel_common_left (S := T) s' s m)
            have hcancel2 :
                LocalizedModule.mk ((s : B) • m') (s * s') =
                  LocalizedModule.mk m' s' := by
              simpa only [Submonoid.smul_def, mul_comm] using
            ...
            have hcancel2T :
              ...
              simpa only [mul_comm] using
                (LocalizedModule.mk_cancel_common_left (S := T) s s' m')
            simp only [hcancel1, hcancel2, hcancel1T, hcancel2T]
            simp only [Submonoid.smul_def]
            abel
      have hEfun_smul (a : A) (x : LocalizedModule T M) :
          Efun (a • x) = a • Efun x := by
        induction x using LocalizedModule.induction_on with
        | _ m s =>
            rw [LocalizedModule.smul'_mk (R := B) (S := T) (R₀ := A) a m s]
            rw [hEfun_mk, hEfun_mk]
            change (LocalizedModule.divBy s)
                (LocalizedModule.mkLinearMap T N (D.1 (a • m)) -
                  (Ecomm (s : B)).1 (LocalizedModule.mk (a • m) s)) =
              a • (LocalizedModule.divBy s)
                (LocalizedModule.mkLinearMap T N (D.1 m) -
                  (Ecomm (s : B)).1 (LocalizedModule.mk m s))
            let divByA : LocalizedModule T N →ₗ[A] LocalizedModule T N :=
              (LocalizedModule.divBy s).restrictScalars A
            change divByA
                (LocalizedModule.mkLinearMap T N (D.1 (a • m)) -
                  (Ecomm (s : B)).1 (LocalizedModule.mk (a • m) s)) =
              a • divByA
            ...
            congr 1
            rw [D.1.map_smul]
            change LocalizedModule.mk (a • D.1 m) 1 -
                (Ecomm (s : B)).1 (LocalizedModule.mk (a • m) s) =
              a • (LocalizedModule.mk (D.1 m) 1 -
                (Ecomm (s : B)).1 (LocalizedModule.mk m s))
            rw [← LocalizedModule.smul'_mk (R := B) (S := T) (R₀ := A) a (D.1 m) 1]
            rw [← LocalizedModule.smul'_mk (R := B) (S := T) (R₀ := A) a m s]
            rw [(Ecomm (s : B)).1.map_smul]
            rw [smul_sub]
      let EfunLinear : LocalizedModule T M →ₗ[A] LocalizedModule T N :=
        { toFun := Efun
          map_add' := hEfun_add
          map_smul' := hEfun_smul }
      have hEcomm_one (m : M) :
          (Ecomm (1 : B)).1 (LocalizedModule.mkLinearMap T M m) = 0 := by
        rw [hEcomm (1 : B) m]
        simp [Dcomm, differentialOperatorCommutator]
      have hdiv_one (x : LocalizedModule T N) :
          LocalizedModule.divBy (1 : T) x = x := by
        induction x using LocalizedModule.induction_on with
        | _ n t =>
            rw [LocalizedModule.divBy_apply, LocalizedModule.liftOn_mk]
            change LocalizedModule.mk n (t * (1 : T)) = LocalizedModule.mk n t
            simp
      have hEfun_extends (m : M) :
          EfunLinear (LocalizedModule.mkLinearMap T M m) =
            LocalizedModule.mkLinearMap T N (D.1 m) := by
        change Efun (LocalizedModule.mk m 1) =
          LocalizedModule.mkLinearMap T N (D.1 m)
        rw [hEfun_mk]
        simp only [raw]
        have hEcomm_one' :
            (Ecomm (1 : B)).1 (LocalizedModule.mk m (1 : T)) = 0 := by
          simpa only [LocalizedModule.mkLinearMap_apply] using hEcomm_one m
        rw [hEcomm_one']
        rw [sub_zero, hdiv_one]
      have hcomm_ext (b : B) :
          differentialOperatorCommutator (R := A) (S := B) EfunLinear b =
            (Ecomm b).1 := by
        apply LinearMap.ext
        intro x
        induction x using LocalizedModule.induction_on with
        | _ m s =>
            let x : LocalizedModule T M := LocalizedModule.mk m s
            have hsx : (s : B) • x =
                LocalizedModule.mkLinearMap T M m := by
              change (s : B) • LocalizedModule.mk m s =
                LocalizedModule.mk m 1
              rw [LocalizedModule.smul'_mk (R := B) (S := T) (R₀ := B)
                (s : B) m s]
              exact LocalizedModule.mk_cancel (R := B) (S := T) s m
            have hprod_eval (u v : B) :
                (Ecomm (u * v)).1 x =
                  (Ecomm u).1 ((v : B) • x) +
                    (u : B) • (Ecomm v).1 x := by
              have hp := congrArg (fun q => q.1 x) (hprod u v)
              simpa only [LinearMap.add_apply,
                LinearMap.comp_apply, mulM, mulN,
                DistribSMul.toLinearMap_apply] using hp
            have hprod_eval_comm :
                (Ecomm b).1 ((s : B) • x) +
                    (b : B) • (Ecomm (s : B)).1 x =
                  (Ecomm (s : B)).1 ((b : B) • x) +
                    (s : B) • (Ecomm b).1 x := by
              calc
                (Ecomm b).1 ((s : B) • x) +
                      (b : B) • (Ecomm (s : B)).1 x =
                    (Ecomm (b * (s : B))).1 x :=
                  (hprod_eval b (s : B)).symm
                _ = (Ecomm ((s : B) * b)).1 x := by rw [mul_comm]
                _ = (Ecomm (s : B)).1 ((b : B) • x) +
                      (s : B) • (Ecomm b).1 x :=
                  hprod_eval (s : B) b
            have hnum :
                (LocalizedModule.mkLinearMap T N (D.1 ((b : B) • m)) -
                    (Ecomm (s : B)).1
                      (LocalizedModule.mk ((b : B) • m) s)) -
                      (b : B) •
                      (LocalizedModule.mkLinearMap T N (D.1 m) -
                        (Ecomm (s : B)).1 (LocalizedModule.mk m s)) =
                  (s : B) • (Ecomm b).1 x := by
              have hDcomm_loc :
                  LocalizedModule.mkLinearMap T N (D.1 ((b : B) • m)) -
                    (b : B) • LocalizedModule.mkLinearMap T N (D.1 m) =
                  LocalizedModule.mkLinearMap T N ((Dcomm b).1 m) := by
                rw [← (LocalizedModule.mkLinearMap T N).map_smul]
                rw [← (LocalizedModule.mkLinearMap T N).map_sub]
                congr 1
              have hbx :
                  LocalizedModule.mk ((b : B) • m) s = (b : B) • x := by
                exact (LocalizedModule.smul'_mk (R := B) (S := T)
                  (R₀ := B) (b : B) m s).symm
              have hEcomm_num :
                  (Ecomm (s : B)).1 ((b : B) • x) -
                      (b : B) • (Ecomm (s : B)).1 x =
              ...
              calc
                (LocalizedModule.mkLinearMap T N (D.1 ((b : B) • m)) -
                    ...
                    (LocalizedModule.mkLinearMap T N (D.1 ((b : B) • m)) -
                      (b : B) • LocalizedModule.mkLinearMap T N (D.1 m)) -
                    ((Ecomm (s : B)).1 ((b : B) • x) -
                      (b : B) • (Ecomm (s : B)).1 x) := by
                  simp only [smul_sub]
                  abel
                _ = LocalizedModule.mkLinearMap T N ((Dcomm b).1 m) -
                    ((Ecomm (s : B)).1 ((b : B) • x) -
                      (b : B) • (Ecomm (s : B)).1 x) := by
                  rw [hDcomm_loc]
                _ = LocalizedModule.mkLinearMap T N ((Dcomm b).1 m) -
                    ((Ecomm b).1 ((s : B) • x) -
                      (s : B) • (Ecomm b).1 x) := by
                  rw [hEcomm_num]
                _ = (s : B) • (Ecomm b).1 x := by
                  rw [hsx, hEcomm b m]
                  abel
            change Efun ((b : B) • x) - (b : B) • Efun x =
              (Ecomm b).1 x
            rw [show (b : B) • x = LocalizedModule.mk ((b : B) • m) s by
              exact LocalizedModule.smul'_mk (R := B) (S := T) (R₀ := B)
                (b : B) m s]
            rw [hEfun_mk]
            have hEfun_x : Efun x = raw (m, s) := by
              simpa only [x] using hEfun_mk m s
            rw [hEfun_x]
            simp only [raw]
            rw [← (LocalizedModule.divBy s).map_smul]
            have hdiv_sub :
                (LocalizedModule.divBy s)
                      (LocalizedModule.mkLinearMap T N (D.1 ((b : B) • m)) -
                        (Ecomm (s : B)).1
                          (LocalizedModule.mk ((b : B) • m) s)) -
                    (LocalizedModule.divBy s)
                    ...
                    ((LocalizedModule.mkLinearMap T N (D.1 ((b : B) • m)) -
                      ...
                      (b : B) •
                        (LocalizedModule.mkLinearMap T N (D.1 m) -
                          (Ecomm (s : B)).1 (LocalizedModule.mk m s))) := by
              symm
              exact (LocalizedModule.divBy s).map_sub
                (LocalizedModule.mkLinearMap T N (D.1 ((b : B) • m)) -
                  (Ecomm (s : B)).1
                    (LocalizedModule.mk ((b : B) • m) s))
                ((b : B) •
            ...
            rw [hdiv_sub, hnum]
            have hs_mul :
                algebraMap B (Module.End B (LocalizedModule T N)) (s : B)
                    ((Ecomm b).1 x) = (s : B) • (Ecomm b).1 x := by
              simp [Module.algebraMap_end_apply]
            rw [← hs_mul]
            simp only [LocalizedModule.divBy_mul_by]
      have hEop :
          IsDifferentialOperator (R := A) (S := B) (k + 1) EfunLinear := by
        intro b
        rw [hcomm_ext b]
        exact (Ecomm b).2
      let E : differentialOperatorSubmodule (R := A) (S := B) (k + 1)
          (M := LocalizedModule T M) (N := LocalizedModule T N) :=
        ⟨EfunLinear, hEop⟩
      refine ⟨E, ?_, ?_⟩
      · intro m
        exact hEfun_extends m
      · intro E' hE'
        apply Subtype.ext
        apply LinearMap.ext
        intro x
        ...
        | _ m s =>
            ...
            have hsx : (s : B) • x =
              ...
              exact LocalizedModule.mk_cancel (R := B) (S := T) s m
            have hcomm_eq (t : B) :
                differentialOperatorCommutator (R := A) (S := B) E'.1 t =
                  (Ecomm t).1 := by
              let C : differentialOperatorSubmodule (R := A) (S := B) k
                  (M := LocalizedModule T M) (N := LocalizedModule T N) :=
                ⟨differentialOperatorCommutator (R := A) (S := B) E'.1 t,
                  E'.2 t⟩
              have hC : ∀ m,
                  C.1 (LocalizedModule.mkLinearMap T M m) =
                    LocalizedModule.mkLinearMap T N ((Dcomm t).1 m) := by
                intro m
                change E'.1 ((t : B) • LocalizedModule.mk m (1 : T)) -
                    (t : B) • E'.1 (LocalizedModule.mk m (1 : T)) =
                  LocalizedModule.mkLinearMap T N ((Dcomm t).1 m)
                rw [LocalizedModule.smul'_mk (R := B) (S := T) (R₀ := B)
                  (t : B) m 1]
                have hE'tm :
                    E'.1 (LocalizedModule.mk (t • m) (1 : T)) =
                      LocalizedModule.mkLinearMap T N (D.1 (t • m)) := by
                  simpa only [LocalizedModule.mkLinearMap_apply] using hE' (t • m)
                have hE'm :
                    E'.1 (LocalizedModule.mk m (1 : T)) =
                      LocalizedModule.mkLinearMap T N (D.1 m) := by
                  simpa only [LocalizedModule.mkLinearMap_apply] using hE' m
                rw [hE'tm, hE'm]
                rw [← (LocalizedModule.mkLinearMap T N).map_smul]
                rw [← (LocalizedModule.mkLinearMap T N).map_sub]
                congr 1
              have hCE : C = Ecomm t := by
                apply (Classical.choose_spec (ih (Dcomm t))).2
                exact hC
              exact congrArg Subtype.val hCE
            have hbase :
                E'.1 (LocalizedModule.mkLinearMap T M m) =
                  EfunLinear (LocalizedModule.mkLinearMap T M m) := by
              rw [hE' m, hEfun_extends m]
            have hmul :
                (s : B) • E'.1 x = (s : B) • EfunLinear x := by
              have hE'comm :
                  differentialOperatorCommutator (R := A) (S := B) E'.1 s =
                    (Ecomm s).1 := hcomm_eq s
              have hEcommE := hcomm_ext s
              calc
                (s : B) • E'.1 x =
                    E'.1 ((s : B) • x) -
                      differentialOperatorCommutator (R := A) (S := B)
                        E'.1 s x := by
                  simp [differentialOperatorCommutator, sub_eq_add_neg]
                _ = E'.1 (LocalizedModule.mkLinearMap T M m) -
                      (Ecomm s).1 x := by rw [hsx, hE'comm]
                _ = EfunLinear (LocalizedModule.mkLinearMap T M m) -
                      (Ecomm s).1 x := by rw [hbase]
                _ = EfunLinear ((s : B) • x) -
                      differentialOperatorCommutator (R := A) (S := B)
                        EfunLinear s x := by rw [hsx, hEcommE]
                _ = (s : B) • EfunLinear x := by
                  simp [differentialOperatorCommutator, sub_eq_add_neg]
            have hmul' :
                algebraMap B (Module.End B (LocalizedModule T N)) (s : B)
                    (E'.1 x) =
                  algebraMap B (Module.End B (LocalizedModule T N)) (s : B)
                    (EfunLinear x) := by
              simpa [Module.algebraMap_end_apply] using hmul
            have hcancel := congrArg (LocalizedModule.divBy s) hmul'
            simpa only [LocalizedModule.divBy_mul_by] using hcancel
/-
  revert D
  induction k with
  | zero =>
