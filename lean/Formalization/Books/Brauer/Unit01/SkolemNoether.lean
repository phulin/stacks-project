...
theorem skolem_noether (k A B : Type*) [Field k] [Ring A] [Algebra k A]
  ...
  let finite_fC : @Module.Finite C M _ _ module_fC := by
    letI : Module C M := module_fC
    letI : IsScalarTower k C M := tower_fC
    exact Module.Finite.of_restrictScalars_finite k C M
  let finite_gC : @Module.Finite C M _ _ module_gC := by
    letI : Module C M := module_gC
    letI : IsScalarTower k C M := tower_gC
    exact Module.Finite.of_restrictScalars_finite k C M
  letI : Ring C := inferInstance
  letI : Semiring C := (inferInstance : Ring C).toSemiring
  letI : IsSimpleRing C := tensor_product_simple_of_simple_algebras k B L
