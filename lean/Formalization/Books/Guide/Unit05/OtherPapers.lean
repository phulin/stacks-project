...
theorem lieblich_associated_points_and_purity_on_artin_stacks
  ...
  exact ⟨{ associatedPoints := hArtin = hArtin, purityOfSheaves := hArtin = hArtin }⟩
...
theorem rydh_noetherian_approximation_of_quasi_finite_diagonal
  ...
  exact ⟨⟨X, 𝟙 X⟩, ⟨{ approximationIsNoetherian := (hqc = hqc) ∧ (hArtin = hArtin), approximatesOriginalStack := hdiagonal = hdiagonal }⟩⟩
