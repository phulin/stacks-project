...
theorem leftRegularGSet_action (G : Type u) [Group G] (g x : G) :
    (Action.leftRegular G).ρ g x = g * x := by
  change g • x = g * x
  rfl
...
def countableGSetPrecoverage (G : Type u) [Group G] [Countable G] :
    Precoverage (CountableGSet G) where
  coverings U := {R | countableGSetCovering (U := U) R}

theorem mem_countableGSetPrecoverage_iff (G : Type u) [Group G] [Countable G]
    {U : CountableGSet G} {R : Presieve U} :
    R ∈ countableGSetPrecoverage G U ↔ countableGSetCovering R := by
  rfl

/-- The countable `G`-sets have pullbacks, since their underlying pullback is
countable as a subset of the product of two countable sets. -/
noncomputable instance countableGSet_hasPullbacks (G : Type u) [Group G]
    [Countable G] : HasPullbacks (CountableGSet G) := by
  sorry

/-- The countable special-case site from the source: its covering presieves
are exactly those generated by countable jointly-surjective families. -/
def countableGSetSite (G : Type u) [Group G] [Countable G] :
    Site (CountableGSet G) where
  coverings U := countableGSetPrecoverage G U
  has_isos := by
    sorry
  pullbacks := by
    sorry
...
theorem countableGSetSite_mem_iff (G : Type u) [Group G] [Countable G]
    {U : CountableGSet G} {R : Presieve U} :
    R ∈ countableGSetSite G U ↔ countableGSetCovering R := by
  rfl
...
/-!
`CountableGSetFamily` records the source's indexed covering data, while
`countableGSetSite` supplies the corresponding countable special-case site.
The ambient universe-bounded `groupActionSite` is the unrestricted set-sized
version used by later declarations.
-/
