...
import Formalization.Books.Sheaves.Unit23.AlgebraicStructures
import Formalization.Books.Sheaves.Unit26.RingedSpaceModules
import Formalization.Books.Sheaves.Unit29.LimitsSheaves
...
/-!
# Sheaves on Spaces, Chapter 30: Bases and sheaves

The basis is represented by an indexed family of opens with the induced
category and induced Grothendieck topology.  This is the canonical Mathlib
subsite formalism; the predicates and structures below expose the covering,
stalk, algebraic-structure, module, and basis-checked `f`-map statements in
the source's order.
-/

namespace Formalization.Books.Sheaves.Unit30
...
open Formalization.Books.Sheaves.Unit23
...
/-- A presheaf of sets on the family `B`. -/
...
/-- The Grothendieck topology induced on the basis. -/
...
/-- The underlying-presheaf comparison for restriction to a basis. -/
noncomputable def basisRestrictionUnderlyingIso {X : TopCat.{v}} {ι : Type v}
    ...
    basisRestrictionFunctor B hB ⋙ CategoryTheory.sheafToPresheaf (basisTopology B) (Type v) ≅
      TopCat.Sheaf.forget (Type v) X ⋙
        (Functor.whiskeringLeft (basisIndex B)ᵒᵖ (Opens X)ᵒᵖ (Type v)).obj
          (inducedFunctor B).op := by
  have : (inducedFunctor B).IsCoverDense (Opens.grothendieckTopology X) :=
    TopCat.Opens.coverDense_inducedFunctor hB
...
/-- Restriction to a topological basis is an equivalence. -/
...
/-- Identity restriction maps satisfy the presheaf identity law. -/
...
/-- Restriction maps satisfy the presheaf composition law. -/
...
/-- Extension of a sheaf on a basis to the whole space. -/
...
/-- Restriction followed by extension recovers the basis sheaf. -/
...
/-- Extension followed by restriction recovers the ordinary sheaf. -/
...
/-- The equivalence between sheaves on a space and sheaves on its basis. -/
...
/-- A selected collection of covers for every basis member. -/
...
/-- It is enough to test the sheaf condition on a cofinal cover system. -/
...
/-- The usual intersection test when the basis is closed under intersections
...
/-- The directed diagram of a basis presheaf around a point. -/
...
/-- The underlying presheaf for a category-valued basis presheaf. -/
...
def BasisAlgebraicSheaf {C : Type u} [Category.{v} C]
    {X : TopCat.{v}} {ι : Type v} (B : ι → Opens X)
    (U : C ⥤ Type v) [AlgebraicStructureType C U]
...
/-- Extension of a structure-valued sheaf from a basis. -/
...
/-- Structure-valued basis sheaves admit an equivalence with ordinary
structure-valued sheaves. -/
noncomputable def basisAlgebraicSheafEquivalence {C : Type u} [Category.{v} C]
    [HasLimits C]
    {X : TopCat.{v}} {ι : Type v} (B : ι → Opens X)
    (hB : Opens.IsBasis (Set.range B)) :
    AlgebraicSheaf C X ≌ CategoryTheory.Sheaf (basisTopology B) C := by
  letI : (basisAlgebraicRestrictionFunctor (C := C) B hB).IsEquivalence :=
    basisAlgebraicRestrictionFunctor_isEquivalence (C := C) B hB
  exact (basisAlgebraicRestrictionFunctor (C := C) B hB).asEquivalence
...
theorem basisAlgebraicExtension_unique {C : Type u} [Category.{v} C]
    [HasLimits C] {X : TopCat.{v}} {ι : Type v} (B : ι → Opens X)
    (hB : Opens.IsBasis (Set.range B))
...
def BasisRingSheaf {X : TopCat.{v}} {ι : Type v} (B : ι → Opens X)
  ...
  by
    letI : AlgebraicStructureType RingCat (CategoryTheory.forget RingCat) :=
      standardAlgebraicStructureTypes.{v, v, v}.2.2.2.2.1
    exact BasisAlgebraicSheaf B (CategoryTheory.forget RingCat) O
...
noncomputable def basisRingSheafExtension {X : TopCat.{v}} {ι : Type v}
  ...
  by
    letI : AlgebraicStructureType RingCat (CategoryTheory.forget RingCat) :=
      standardAlgebraicStructureTypes.{v, v, v}.2.2.2.2.1
    exact basisAlgebraicExtension B hB O hO
...
noncomputable def basisRingSheafExtensionRestrictionIso
    ...
    (basisAlgebraicRestrictionFunctor (C := RingCat) B hB).obj
      ...
      ⟨O, (CategoryTheory.Presheaf.isSheaf_iff_isSheaf_forget
        ...
        (CategoryTheory.forget RingCat)).2 hO⟩ := by
  letI : AlgebraicStructureType RingCat (CategoryTheory.forget RingCat) :=
    standardAlgebraicStructureTypes.{v, v, v}.2.2.2.2.1
  exact basisAlgebraicExtensionRestrictionIso B hB O hO
...
/-- The underlying additive presheaf of a basis-module presheaf. -/
...
def basisModuleAction {X : TopCat.{v}} {ι : Type v}
    {B : ι → Opens X} {O : BasisRingPresheaf B}
    (F : BasisModulePresheaf B O) (i : basisIndex B) :
    O.obj (op i) → F.obj (op i) → F.obj (op i) :=
  fun r m => r • m

/-- The basis-module action commutes with restriction maps. -/
theorem basisModuleAction_map {X : TopCat.{v}} {ι : Type v}
    {B : ι → Opens X} {O : BasisRingPresheaf B}
    (F : BasisModulePresheaf B O) {i j : basisIndex B} (f : i ⟶ j)
    (r : O.obj (op j)) (m : F.obj (op j)) :
    F.map f.op (basisModuleAction F j r m) =
      basisModuleAction F i (O.map f.op r) (F.map f.op m) := by
  exact F.map_smul f.op r m

/-- The underlying type of the categorical module stalk. -/
...
/-- The module stalk agrees with the stalk of the underlying additive presheaf. -/
...
/-- The canonical module structure on a basis-module stalk. -/
@[instance_reducible]
noncomputable def basisModuleStalkModule {X : TopCat.{v}} {ι : Type v}
    (B : ι → Opens X) (hB : Opens.IsBasis (Set.range B))
    {O : BasisRingPresheaf B} (F : BasisModulePresheaf B O) (x : X) :
    Module ((colimit ((ObjectProperty.ι (fun i : basisIndex B => x ∈ B i)).op ⋙ O) :
      RingCat.{v}) : Type v)
  ...
  letI (i : (basisNeighborhoodIndex B x)ᵒᵖ) :
    ...
    change Module (O.obj (op (unop i).obj)) (F.presheaf.obj (op (unop i).obj))
    infer_instance
...
/-- Data witnessing extension of a basis-module sheaf. -/
...
theorem exists_basisModuleExtension {X : TopCat.{v}} {ι : Type v}
    ...
    Nonempty (BasisModuleExtensionData B hB O F hF) := by sorry
...
/-- Extension of a sheaf of modules from a basis. -/
...
/-- The selected extension restricts to the given basis module. -/
...
/-- The canonical sectionwise action on the extended module sheaf. -/
...
/-- The extended action commutes with restriction maps. -/
...
theorem basisModuleExtension_stalk_module_iso {X : TopCat.{v}} {ι : Type v}
  ...
  let SM := (ModuleCat.restrictScalars eR.hom.hom).obj
    ((Formalization.Books.Sheaves.Unit22.moduleStalkFunctor O x).obj M)
  ...
  have hsmul : ∀ r, ha.hom ≫ TM.smul r = SM.smul r ≫ ha.hom := by
    ...
    have hm₂ :
      ...
      have h_inj : Function.Injective (ConcreteCategory.hom ha.hom) := by
        intro a b hab
        have hab' := congrArg (fun q => (ConcreteCategory.hom ha.inv) q) hab
        simpa using hab'
      apply h_inj
      dsimp [m₂, aIso, cQ, fQ, cQR, qK, qr, rA, q, P, Gaddx, Gadd]
      have hf_inv :
      ...
      have hcomp' := congrArg (fun q => (ConcreteCategory.hom q) m₂) hcomp
      have hcomp'' :
      ...
      have hcQR'' :
          (ConcreteCategory.hom cQR.hom)
              ((ConcreteCategory.hom
                (colimit.ι
                  ((ObjectProperty.ι (fun i : basisIndex B => x ∈ B i)).op ⋙ P) k))
                ((ConcreteCategory.hom (qK.inv.app k)) m₂)) =
...
noncomputable def basisModuleSheafEquivalence {X : TopCat.{v}} {ι : Type v}
    (B : ι → Opens X) (hB : Opens.IsBasis (Set.range B))
    (O : RingSheaf.{v, v} X) :
    Mod O ≌ BasisModuleSheafCategory.{v, v} B O := by
  ...
  let : R.Faithful := by
    ...
    apply (PresheafOfModules.toPresheaf O.1).map_injective
    apply TopCat.Sheaf.hom_ext M.val.presheaf
      ⟨N.val.presheaf, N.isSheaf⟩ hB
    intro i
    have h' := congrArg (fun q => q.hom) h
    have hi := congrArg (fun q => q.app (op i)) h'
...
theorem basisFMap_below_unique {X Y : TopCat.{v}} {κ : Type v}
  ...
  let _ : (inducedFunctor Bᵧ).IsCoverDense (Opens.grothendieckTopology Y) :=
    TopCat.Opens.coverDense_inducedFunctor hBᵧ
  have hbij := Functor.whiskerLeft_obj_map_bijective_of_isCoverDense
    (K := Opens.grothendieckTopology Y) (inducedFunctor Bᵧ) G.presheaf
    ((TopCat.Sheaf.pushforward C f).obj F).obj
    ((TopCat.Sheaf.pushforward C f).obj F).property
  rcases hbij.2 φ with ⟨a, ha⟩
  let ψ : AlgebraicFMap f G F :=
    (fullyFaithfulSheafToPresheaf (Opens.grothendieckTopology Y) C).preimage a
  refine ⟨ψ, ?_, ?_⟩
  · change ((Functor.whiskeringLeft (basisIndex Bᵧ)ᵒᵖ (Opens Y)ᵒᵖ C).obj
        (inducedFunctor Bᵧ).op).map ψ.hom = φ
    exact ha
  · intro ψ' hψ'
    have hraw : ((Functor.whiskeringLeft (basisIndex Bᵧ)ᵒᵖ (Opens Y)ᵒᵖ C).obj
        (inducedFunctor Bᵧ).op).map ψ'.hom =
        ((Functor.whiskeringLeft (basisIndex Bᵧ)ᵒᵖ (Opens Y)ᵒᵖ C).obj
          (inducedFunctor Bᵧ).op).map a := hψ'.trans ha.symm
    have hraw' := hbij.1 hraw
    apply (fullyFaithfulSheafToPresheaf (Opens.grothendieckTopology Y) C).map_injective
    exact hraw'
...
theorem basisFMap_below_unique_of_data {X Y : TopCat.{v}} {κ : Type v}
  ...
  let φ : (inducedFunctor Bᵧ).op ⋙ G.presheaf ⟶
      (inducedFunctor Bᵧ).op ⋙ ((TopCat.Sheaf.pushforward C f).obj F).obj :=
    { app := fun U => d.map U.unop
      naturality := by
        intro U V h
        change G.presheaf.map _ ≫ d.map _ = d.map _ ≫ F.presheaf.map _
        exact d.compatible h.unop.hom.le }
  obtain ⟨ψ, hψ, huniq⟩ := basisFMap_below_unique f F G Bᵧ hBᵧ φ
  refine ⟨ψ, ?_, ?_⟩
  · intro j
    have hj := congr_app hψ (op (show basisIndex Bᵧ from j))
    change ψ.hom.app (op (Bᵧ j)) = d.map j at hj
    exact hj
  · intro ψ' hψ'
    apply huniq ψ'
    ext U
...
/-- The target-basis uniqueness statement for module-valued `f`-maps. -/
theorem basisFMapModule_below_unique {X Y : RingedSpace} {κ : Type v}
  ...
  let φAdd :=
    (PresheafOfModules.toPresheaf ((inducedFunctor Bᵧ).op ⋙ Y.structureSheaf.1)).map φ
  let α : G.val.presheaf ⟶ T.val.presheaf :=
    TopCat.Sheaf.restrictHomEquivHom G.val.presheaf
      ⟨T.val.presheaf, T.isSheaf⟩ hBᵧ φAdd
  let fval : G.val ⟶ T.val := PresheafOfModules.homMk α (by
    intro U r m
    let I := {i : basisIndex Bᵧ // Bᵧ i ≤ U.unop}
    let A : I → Opens Y := fun i => Bᵧ i.1
    have hA : iSup A = U.unop := by
      apply le_antisymm
      · exact iSup_le fun i => i.2
      · intro x hx
        obtain ⟨V, ⟨i, rfl⟩, hxV, hi⟩ := (Opens.isBasis_iff_nbhd.mp hBᵧ) hx
        exact Opens.mem_iSup.mpr ⟨⟨i, hi⟩, hxV⟩
    let R : Presieve U.unop := TopCat.Presheaf.presieveOfCoveringAux A U.unop
    have hR : Sieve.generate R ∈ Opens.grothendieckTopology Y U.unop := by
      change Sieve.generate
        (TopCat.Presheaf.presieveOfCoveringAux A U.unop) ∈
          Opens.grothendieckTopology Y U.unop
      exact hA ▸ TopCat.Presheaf.presieveOfCovering.mem_grothendieckTopology A
    have hTtype : Presheaf.IsSheaf (Opens.grothendieckTopology Y)
        (T.val.presheaf ⋙ CategoryTheory.forget AddCommGrpCat) :=
      (CategoryTheory.Presheaf.isSheaf_iff_isSheaf_forget
        (J := Opens.grothendieckTopology Y) (P' := T.val.presheaf)
        (CategoryTheory.forget AddCommGrpCat)).mp T.isSheaf
    have hTsheaf : Presieve.IsSheaf (Opens.grothendieckTopology Y)
        (T.val.presheaf ⋙ CategoryTheory.forget AddCommGrpCat) :=
      (CategoryTheory.isSheaf_iff_isSheaf_of_type
        (J := Opens.grothendieckTopology Y)
        (T.val.presheaf ⋙ CategoryTheory.forget AddCommGrpCat)).mp hTtype
    have hsep : Presieve.IsSeparatedFor
        (T.val.presheaf ⋙ CategoryTheory.forget AddCommGrpCat) R :=
      (hTsheaf.isSheafFor R hR).isSeparatedFor
    apply hsep.ext
      (t₁ := (α.app U) (r • m)) (t₂ := r • (α.app U) m)
    intro V fV hf
    obtain ⟨i, rfl⟩ := hf
    have hαi := TopCat.Sheaf.extend_hom_app G.val.presheaf
      ⟨T.val.presheaf, T.isSheaf⟩ hBᵧ φAdd i.1
    have hn := α.naturality fV.op
    have hn' := congrArg (fun q =>
      (ConcreteCategory.hom q) ((r • m : G.val.obj U) : G.val.presheaf.obj U)) hn
    change (ConcreteCategory.hom (T.val.presheaf.map fV.op))
        ((ConcreteCategory.hom (α.app (op (unop U))))
          ((r • m : G.val.obj U) : G.val.presheaf.obj (op (unop U)))) =
      (ConcreteCategory.hom (T.val.presheaf.map fV.op))
        ((r • (ConcreteCategory.hom (α.app (op (unop U)))) m : T.val.obj U) :
          T.val.presheaf.obj (op (unop U)))
    have hn'' := hn'.symm
    have hleft :
        (ConcreteCategory.hom
            ((T.val.presheaf ⋙ CategoryTheory.forget AddCommGrpCat).map fV.op))
            ((ConcreteCategory.hom (α.app U))
              ((r • m : G.val.obj U) : G.val.presheaf.obj U)) =
          (ConcreteCategory.hom (α.app (op (A i))))
            ((ConcreteCategory.hom (G.val.presheaf.map fV.op))
              ((r • m : G.val.obj U) : G.val.presheaf.obj U)) := by
      exact hn''
    have hφ' : ∀ z : G.val.presheaf.obj (op (unop U)),
        (α.app (op (A i))) ((G.val.presheaf.map fV.op) z) =
          (T.val.presheaf.map fV.op) ((α.app (op (unop U))) z) :=
      fun z => CategoryTheory.congr_fun (α.naturality fV.op) z
    rw [← hφ' ((r • m : G.val.obj U) : G.val.presheaf.obj (op (unop U)))]
    have hG := G.val.map_smul fV.op r m
    have hG' := congrArg (fun q =>
      (ConcreteCategory.hom (α.app (op (A i))))
        ((q : G.val.obj (op (A i))) : G.val.presheaf.obj (op (A i)))) hG
    have hT := T.val.map_smul fV.op r ((α.app (op (unop U))) m)
    let hm0 : (basisModuleRestriction Bᵧ G.val).obj (op i.1) :=
      (ConcreteCategory.hom (G.val.map fV.op)) m
    let hr : ((inducedFunctor Bᵧ).op ⋙ Y.structureSheaf.1).obj (op i.1) :=
      (ConcreteCategory.hom (Y.structureSheaf.1.map fV.op)) r
    let hr0 : Y.structureSheaf.1.obj (op (A i)) :=
      (ConcreteCategory.hom (Y.structureSheaf.1.map fV.op)) r
    have hq :
        (((ConcreteCategory.hom (Y.structureSheaf.1.map fV.op)) r) •
            ((ConcreteCategory.hom (G.val.map fV.op)) m) :
          G.val.presheaf.obj (op (A i))) =
        ((hr • hm0 : (basisModuleRestriction Bᵧ G.val).obj (op i.1)) :
          G.val.presheaf.obj (op (A i))) := by
      dsimp [hr, hm0, basisModuleRestriction]
      rfl
    have hφi := (φ.app (op i.1)).hom.map_smul hr hm0
    change (ConcreteCategory.hom (α.app (op (A i))))
        ((ConcreteCategory.hom (G.val.map fV.op)) (r • m)) =
      (ConcreteCategory.hom (T.val.map fV.op))
        (r • (ConcreteCategory.hom (α.app (op (unop U)))) m)
    rw [hG']
    rw [show (α.app (op (A i))) =
        (φAdd.app (op i.1)) by
          exact hαi]
    change (ModuleCat.Hom.hom (φ.app (op i.1))) (hr • hm0) =
      (ConcreteCategory.hom (T.val.map fV.op))
        (r • (ConcreteCategory.hom (α.app (op (unop U)))) m)
    rw [hφi]
    change hr0 • (ModuleCat.Hom.hom (φ.app (op i.1))) hm0 =
      (ConcreteCategory.hom (T.val.map fV.op))
        (r • (ConcreteCategory.hom (α.app (op (unop U)))) m)
    let y : T.val.obj (op (A i)) :=
      (ConcreteCategory.hom (α.app (op (A i))))
        ((hm0 : (basisModuleRestriction Bᵧ G.val).obj (op i.1)) :
          G.val.presheaf.obj (op (A i)))
    have hαm : y =
          (ModuleCat.Hom.hom (φ.app (op i.1))) hm0 := by
      have hαm' := congrArg (fun q => (ConcreteCategory.hom q)
        ((hm0 : (basisModuleRestriction Bᵧ G.val).obj (op i.1)) :
          G.val.presheaf.obj (op (A i))))
        (show α.app (op (A i)) = φAdd.app (op i.1) by exact hαi)
      calc
        _ = (ConcreteCategory.hom (φAdd.app (op i.1)))
            ((hm0 : (basisModuleRestriction Bᵧ G.val).obj (op i.1) ) :
              G.val.presheaf.obj (op (A i))) := hαm'
        _ = _ := by
          exact PresheafOfModules.toPresheaf_map_app_apply φ (op i.1)
            ((hm0 : (basisModuleRestriction Bᵧ G.val).obj (op i.1) ) :
              G.val.presheaf.obj (op (A i)))
    rw [← hαm]
    change hr0 • ((ConcreteCategory.hom (α.app (op (A i))))
        ((ConcreteCategory.hom (G.val.presheaf.map fV.op)) m) :
          T.val.obj (op (A i))) =
      (ConcreteCategory.hom (T.val.map fV.op))
        (r • (ConcreteCategory.hom (α.app (op (unop U)))) m)
    rw [hφ' m]
  ...
  let ψ : RingedSpaceModuleFMap f G F :=
    (SheafOfModules.forget Y.structureSheaf).preimage fval
  have hψ0 : basisModuleRestrictionHom Bᵧ ψ.val = φ := by
    have hpre : ψ.val = fval := by
      change ((SheafOfModules.forget Y.structureSheaf).preimage
        (X := G) (Y := T) fval).val = fval
      exact (SheafOfModules.forget Y.structureSheaf).map_preimage
        (X := G) (Y := T) fval
    rw [hpre]
    apply (PresheafOfModules.toPresheaf
      ((inducedFunctor Bᵧ).op ⋙ Y.structureSheaf.1)).map_injective
    ext U x
    change (ConcreteCategory.hom
        (fval.app ((inducedFunctor Bᵧ).op.obj U))) x =
      (ConcreteCategory.hom (φAdd.app U)) x
    exact congrArg (fun q => (ConcreteCategory.hom q) x)
      (TopCat.Sheaf.extend_hom_app G.val.presheaf
        ⟨T.val.presheaf, T.isSheaf⟩ hBᵧ φAdd U.unop)
  refine ⟨ψ, hψ0, ?_⟩
  · intro ψ' hψ'
    have hres : basisModuleRestrictionHom Bᵧ ψ'.val =
        basisModuleRestrictionHom Bᵧ ψ.val := hψ'.trans hψ0.symm
    apply (SheafOfModules.forget Y.structureSheaf).map_injective
    apply (PresheafOfModules.toPresheaf Y.structureSheaf.1).map_injective
    apply TopCat.Sheaf.hom_ext G.val.presheaf
      ⟨T.val.presheaf, T.isSheaf⟩ hBᵧ
    intro j
    have hj := congrArg (fun q => q.app (op (show basisIndex Bᵧ from j))) hres
    have hj' := congrArg (fun q =>
      (CategoryTheory.forget₂
        (ModuleCat (((inducedFunctor Bᵧ).op ⋙ Y.structureSheaf.1).obj
          (op (show basisIndex Bᵧ from j)))) AddCommGrpCat).map q) hj
    exact hj'
...
/-- A two-basis family of module maps with scalar compatibility. -/
...
/-
  ...
  let GAdd : TopCat.Sheaf AddCommGrpCat Y :=
    ⟨G.val.presheaf ⋙ CategoryTheory.forget AddCommGrpCat, G.isSheaf⟩
  let dAdd : BasisFMapAboveBelowData f Bₓ Bᵧ GAdd FAdd :=
    { app := fun i j hij =>
        AddCommGrpCat.ofHom (AddMonoidHom.mk' (d.app i j hij) (by
          intro s₁ s₂
          exact d.map_add i j hij s₁ s₂))
      naturality := by
        intro i i' j j' hii' hjj' hij hi'j'
        ext s
        change F.val.presheaf.map (homOfLE hii').op (d.app i j hij s) =
          d.app i' j' hi'j' (G.val.presheaf.map (homOfLE hjj').op s)
        exact d.naturality hii' hjj' hij hi'j' s }
  obtain ⟨ψ, hψ, huniq⟩ := basisFMap_above_below_unique f FAdd GAdd Bₓ hBₓ Bᵧ hBᵧ dAdd
  let φ : BasisModuleFMapBelowData f Bᵧ G F :=
    { map := fun j s => (ψ.hom.app (op (Bᵧ j))) s
      compatible := by
        intro j j' h
        funext s
        change (ψ.hom.app (op (Bᵧ j')))
              (G.val.presheaf.map (homOfLE h).op s) =
          F.val.presheaf.map
              (((Opens.map f.continuous).map (homOfLE h)).op)
            ((ψ.hom.app (op (Bᵧ j))) s)
        exact congrFun (ψ.hom.naturality (homOfLE h).op) s
      map_add := by
        intro j s₁ s₂
        exact (ψ.hom.app (op (Bᵧ j))).hom.map_add s₁ s₂
      linear := by
        intro j r s
        let U := (Opens.map f.continuous).obj (Bᵧ j)
        let I := {i : basisIndex Bₓ // Bₓ i ≤ U}
        let A : I → Opens X := fun i => Bₓ i.1
        have hA : iSup A = U := by
          apply le_antisymm
          · exact iSup_le fun i => i.2
          · intro x hx
            obtain ⟨V, ⟨i, rfl⟩, hxV, hi⟩ :=
              (Opens.isBasis_iff_nbhd.mp hBₓ) hx
            exact Opens.mem_iSup.mpr ⟨⟨i, hi⟩, hxV⟩
        let R : Presieve U := TopCat.Presheaf.presieveOfCoveringAux A U
        have hR : Sieve.generate R ∈ Opens.grothendieckTopology X U := by
          change Sieve.generate
            (TopCat.Presheaf.presieveOfCoveringAux A U) ∈
              Opens.grothendieckTopology X U
          exact hA ▸ TopCat.Presheaf.presieveOfCovering.mem_grothendieckTopology A
        have hFtype : Presheaf.IsSheaf (Opens.grothendieckTopology X)
            (F.val.presheaf ⋙ CategoryTheory.forget AddCommGrpCat) :=
          (CategoryTheory.Presheaf.isSheaf_iff_isSheaf_forget
            (J := Opens.grothendieckTopology X) (P' := F.val.presheaf)
            (CategoryTheory.forget AddCommGrpCat)).mp F.val.isSheaf
        have hFsheaf : Presieve.IsSheaf (Opens.grothendieckTopology X)
            (F.val.presheaf ⋙ CategoryTheory.forget AddCommGrpCat) :=
          (CategoryTheory.isSheaf_iff_isSheaf_of_type
            (J := Opens.grothendieckTopology X)
            (F.val.presheaf ⋙ CategoryTheory.forget AddCommGrpCat)).mp hFtype
        have hsep : Presieve.IsSeparatedFor
            (F.val.presheaf ⋙ CategoryTheory.forget AddCommGrpCat) R :=
          (hFsheaf.isSheafFor R hR).isSeparatedFor
        apply hsep.ext
          (t₁ := (ψ.hom.app (op (Bᵧ j))) (r • s))
          (t₂ := (ringedSpaceBasisScalarMap
            (U := U) (V := Bᵧ j) f le_rfl).hom r •
              (ψ.hom.app (op (Bᵧ j))) s)
        intro V fV hf
        obtain ⟨i, rfl⟩ := hf
        let hij : Bₓ i.1 ≤ U := i.2
        have hψr := congrArg (fun q => q (r • s)) (hψ i.1 j hij)
        have hψs := congrArg (fun q => q s) (hψ i.1 j hij)
        change F.val.presheaf.map (homOfLE hij).op
              ((ψ.hom.app (op (Bᵧ j))) (r • s)) =
            d.app i.1 j hij (r • s) at hψr
        change F.val.presheaf.map (homOfLE hij).op
              ((ψ.hom.app (op (Bᵧ j))) s) =
            d.app i.1 j hij s at hψs
        rw [hψr, hψs, d.linear]
        rw [F.val.map_smul]
        congr 1
        change (X.structureSheaf.1.map (homOfLE hij).op)
              ((ringedSpaceBasisScalarMap (U := U) (V := Bᵧ j)
                f le_rfl).hom r) =
          (ringedSpaceBasisScalarMap f hij).hom r
        dsimp [ringedSpaceBasisScalarMap]
        simp only [Category.assoc]
        rfl }
  obtain ⟨ψ', hψ', huniq'⟩ := basisFMapModule_below_unique_of_data
    f F G Bᵧ hBᵧ φ
  refine ⟨ψ', ?_, ?_⟩
  · intro i j hij s
    have hj := hψ' j s
    have hψj := congrArg (fun q => q s) (hψ i j hij)
    change F.val.presheaf.map (homOfLE hij).op
          ((ψ'.val.app (op (Bᵧ j))).hom s) = d.app i j hij s
    exact hj.trans hψj
  · intro ψ'' hψ''
    apply huniq' ψ''
    ext U s
...
end Formalization.Books.Sheaves.Unit30
