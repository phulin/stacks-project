...
/-- The polynomial derivative ideal after extension to the formal power series ring. -/
noncomputable def polynomialPowerSeriesDerivativeIdeal {n : ℕ}
    (p : MvPolynomial (Fin n) k) : Ideal (MvPowerSeries (Fin n) k) :=
  (polynomialDerivativeIdeal k p).map MvPolynomial.coeToMvPowerSeries.ringHom
...
theorem powerSumPolynomial_derivative_zeroLocus_is_singleton
    ...
    ∃ x : PrimeSpectrum (MvPowerSeries (Fin n) k),
      PrimeSpectrum.zeroLocus
          (polynomialPowerSeriesDerivativeIdeal k (powerSumPolynomial k n d)) = {x} := by
  /-
  Roadmap (derivative locus).
  1. Rewrite `polynomialPowerSeriesDerivativeIdeal` with
     `powerSumPolynomial_derivative_ideal`, then use `Ideal.map_span`,
     `MvPolynomial.coeToMvPowerSeries.ringHom_apply`, `MvPolynomial.coe_X`, and
     `MvPowerSeries.pderiv_coe`.  The resulting ideal is generated by
     `(d : k) • MvPowerSeries.X i ^ (d - 1)`.
  2. Let `J := Ideal.span (Set.range (MvPowerSeries.X : Fin n → _))`.  Prove
     `J = RingHom.ker MvPowerSeries.constantCoeff` by transporting
     `AdicCompletion.ker_evalOneₐ_eq_map (MvPolynomial.idealOfVars_fg (Fin n) k)`
     through `MvPowerSeries.toAdicCompletionAlgEquiv` and its compatibility
     lemmas in `Mathlib/RingTheory/MvPowerSeries/Equiv.lean`.
  3. The kernel is maximal by `RingHom.ker_isMaximal_of_surjective`, using
     `MvPowerSeries.constantCoeff_C`.  Every prime in the displayed zero locus
     contains each `X i`: remove the nonzero scalar `d`, then use primality and
     `0 < d - 1`.  Hence it contains `J` and equals `J` by maximality.
  4. Take `x` to be that maximal prime and prove the set equality by
     `PrimeSpectrum.ext` and `Set.ext`.
  -/
  sorry
...
theorem finiteProductSum_derivative_mem_factorIdeal
  ...
  /-
  Roadmap (Leibniz containment).
  Unfold `powerSeriesFactorIdeal`, commute `MvPowerSeries.pderiv` with the finite
  sum via the additive-map lemma `map_sum`, and rewrite every summand with
  `Derivation.leibniz`.  Both `f j` and `g j` are generators of the span by
  `Ideal.subset_span` and membership in the two sides of `Set.range f ∪
  Set.range g`; close each product with `Ideal.mul_mem_left`/`mul_mem_right`,
  then close the sum with `Ideal.sum_mem` from
...
theorem finiteProductSum_factor_locus_dimension
  ...
  /-
  Roadmap (height bound).
  1. Replace the union of the two finite ranges by
     `s := Finset.univ.image f ∪ Finset.univ.image g`; show the factor ideal is
     `Ideal.span (s : Set _)` and `s.card ≤ 2 * t`.
  2. From `hzero` and `MvPowerSeries.isUnit_iff_constantCoeff`, every member of
     `s` is in `IsLocalRing.maximalIdeal`; rewrite this as membership in
     `Ring.jacobson` with `IsLocalRing.ringJacobson_eq_maximalIdeal`.
  3. Prove `((n : ℕ) : WithBot ℕ∞) ≤ ringKrullDim (MvPowerSeries (Fin n) k)` by
     induction on `n`: use `MvPowerSeries.isEmptyEquiv` at zero and transport
     `ringKrullDim_succ_le_ringKrullDim_powerseries` across
     `MvPowerSeries.finSuccEquiv` at the successor.
  4. Apply `ringKrullDim_le_ringKrullDim_quotient_add_card s`.  Combine the
     preceding two inequalities and `s.card ≤ 2*t`; split the quotient
     dimension into its `WithBot` cases and normalize coercions to finish the
     truncated-subtraction inequality.  The quotient is nontrivial because
     the span lies in the proper Jacobson radical, excluding the `⊥` case.
  -/
  sorry
...
theorem finiteProductSum_derivative_locus_contains_factor_locus
  ...
  /-
  Roadmap (reverse inclusion of vanishing loci).
  Prove `powerSeriesDerivativeIdeal k f g ≤ powerSeriesFactorIdeal k f g` with
  `Ideal.span_le`; a range generator is exactly
  `finiteProductSum_derivative_mem_factorIdeal k f g i`.  For a prime `p` in
  the factor zero locus, unpack membership as containment of the factor ideal,
  compose with this inequality, and repack it as membership in the derivative
  zero locus.  `PrimeSpectrum.mem_zeroLocus` (from
  `Mathlib/RingTheory/Spectrum/Prime/Basic.lean`) supplies the conversions.
  -/
  sorry
...
theorem powerSumPolynomial_not_finiteProductSum
  ...
  /-
  Roadmap (assemble the derivative obstruction).
  1. Assume a witness `f,g,hzero,hsum`.  By `MvPowerSeries.pderiv_coe` and
     congruence under `hsum`, identify `powerSeriesDerivativeIdeal k f g` with
     `polynomialPowerSeriesDerivativeIdeal k (powerSumPolynomial k n d)`.
  2. Use `finiteProductSum_factor_locus_dimension`; `hnt` turns its lower bound
     into a positive dimension.  Obtain the ideal inequality derivative ≤
     factor from the proof pattern of
     `finiteProductSum_derivative_locus_contains_factor_locus`.
  3. The quotient factor map associated to that inequality is surjective
     (`Ideal.Quotient.factor_surjective`), so `ringKrullDim_le_of_surjective`
     bounds the factor-locus dimension by the derivative-locus dimension.
  4. Apply `powerSumPolynomial_derivative_zeroLocus_is_singleton`.  Rewrite
     the derivative quotient dimension with `ringKrullDim_quotient`; the
     singleton equality gives `Order.krullDim_eq_zero`.  This contradicts the
     positive lower bound.  Keep both ideals in `MvPowerSeries`; mapping the
     singleton statement back to `MvPolynomial` is the known dead end fixed by
     `polynomialPowerSeriesDerivativeIdeal`.
  -/
  sorry

theorem exists_powerSum_obstruction (t degreeFloor : ℕ) :
    ∃ (n d : ℕ), 2 * t < n ∧ degreeFloor < d ∧ 1 < d ∧ (d : k) ≠ 0 ∧
      ¬ IsFiniteProductSum k t (powerSumPolynomial k n d) := by
  /-
  Roadmap (unbounded characteristic-safe degrees).
  Take `n := 2*t + 1`.  Put `p := ringChar k` and split on `p = 0`.
  In characteristic zero take `d := max degreeFloor 1 + 1`; its cast is
  nonzero by `ringChar.spec`.  If `p > 0`, first obtain `2 ≤ p` from
  `ringChar_eq_one` and nontriviality, then take
  `d := p * (max degreeFloor 1 + 1) + 1`; it is above both bounds and its cast
  is `1`, using `ringChar.Nat.cast_ringChar`.  Arithmetic is discharged after
  these definitions with `omega`.  Finish with
  `powerSumPolynomial_not_finiteProductSum k n d t`.
  This strengthened interface is needed by `exists_homogeneousBlockSeries`:
  the former statement supplied neither arbitrarily high degrees nor the
  `t = 0` case, so it could not produce a strictly increasing obstruction
  sequence.
  -/
  sorry
...
theorem exists_homogeneousBlockSeries :
  ...
  /-
  Roadmap (dependent block recursion).
  1. Choose the zeroth `(width, degree, block)` with
     `exists_powerSum_obstruction k 0 1`.  At successor `i+1`, apply
     `exists_powerSum_obstruction k (i+1) (degree i)`.  Use `Classical.choose`
     inside `Nat.rec` so each new degree is strictly larger than its
     predecessor.
  2. Define `blockStart 0 := 1` and
     `blockStart (i+1) := blockStart i + width i`.  Positivity and strict
     monotonicity follow from `2*i < width i`.
  3. Transport the chosen `powerSumPolynomial` from `Fin (width i)` to
     `Fin (blockStart (i+1)-blockStart i)` along the equality from
     `Nat.add_sub_cancel_left`; use `MvPolynomial.renameEquiv` if Lean does not
     reduce the index types definitionally.  Homogeneity is
     `powerSumPolynomial_isHomogeneous` and is invariant under this rename.
  4. For `t ≤ i`, pad a `t`-term representation with zero pairs to an
     `i`-term representation (rename indices with `Fin.castLE`); constant
     coefficients remain zero.  Contradict the obstruction chosen at `i`.
  -/
  sorry
...
theorem exists_completion_kernel_not_square :
  ...
  /-
  Roadmap (block series in the completion).
  1. Choose `B : HomogeneousBlockSeries k`.  Embed block `i` into variables
     `B.blockStart i, …, B.blockStart (i+1)-1` with `MvPolynomial.rename` and
     form finite partial sums.  Combine
     `MvPolynomial.mem_pow_idealOfVars_iff'` with
     `MvPolynomial.IsHomogeneous.coeff_eq_zero`, `B.degree_gt_one`, and strict
     degrees to prove these sums are an `AdicCompletion.AdicCauchySequence`;
     let `z` be its `AdicCompletion.mk`.
  2. Every block has degree at least two, so `AdicCompletion.evalₐ _ 2 z = 0`;
     this is exactly membership in `completionKernel _ 2` by `RingHom.mem_ker`.
  3. If `z` lies in the square, rewrite `pow_two` and
     `Ideal.span_mul_span` after `Ideal.span_eq` to extract a finite expression
     `z = ∑ j : Fin t, F j * G j` with both factors in
     `completionMaximalIdeal`.  Absorb span coefficients into one factor.
  4. Define the coefficient embedding of the infinite completion into
     `MvPowerSeries ℕ k` using `MvPowerSeries.toAdicCompletionInv`; prove its
     ring laws coefficientwise from `AdicCompletion.evalₐ` at
     `Finsupp.degree q + 1`.  For block `i`, compose it with
     `MvPowerSeries.killCompl` for the interval embedding `Fin width ↪ ℕ`.
     The lemmas `coeff_killCompl`, `killCompl_X`, and
     `MvPowerSeries.pderiv_coe` are in `MvPowerSeries/Rename.lean` and
     `Derivative.lean`.
  5. Choose any `i ≥ t`.  Disjointness of the block intervals makes the
     restriction send `z` to `B.block i`; it sends each factor to a series
     with zero constant coefficient and turns the finite square expression
     into `IsFiniteProductSum k t (B.block i)`, contradicting
     `B.obstruction i t`.  Do not try to use
     `MvPowerSeries.toAdicCompletionAlgEquiv` at index `ℕ`: it requires
     `[Finite ℕ]`; only its coefficient inverse is available here.
  -/
  sorry
...
theorem infinitePolynomial_completion_not_complete_as_completionModule :
  ...
  intro hcomplete
  have hpow :=
    (completion_is_completeAsCompletionModule_iff_kernel_eq_pow
      (infinitePolynomialMaximalIdeal k)).mp hcomplete 2 (by omega)
  obtain ⟨z, hzker, hznot⟩ := exists_completion_kernel_not_square k
  apply hznot
  rw [← hpow]
  exact hzker
...
theorem lemma_noncomplete_completion :
  ...
  /-
  Roadmap (localize and transport the counterexample).
  1. Let `A := infinitePolynomialRing k`, `M :=
     infinitePolynomialMaximalIdeal k`, `S := Localization.AtPrime M`, and
     `m := M.map (algebraMap A S)`.  Use
     `Localization.AtPrime.map_eq_maximalIdeal` and the standard
     `IsLocalRing S` instance; package `S` as `CommRingCat.of S`.
  2. Build compatible ring equivalences `A ⧸ M^n ≃+* S ⧸ m^n`.
     Rewrite powers with `Ideal.map_pow`.  The forward quotient map is
     surjective because every denominator outside `M` becomes a unit modulo
     `M^n`; use `Ideal.Quotient.isUnit_mk_pow_of_notMem` and
     `IsLocalization.of_le_isUnit`, `IsLocalization.atUnits`, and
     `IsLocalization.algEquiv` from
     `Mathlib/RingTheory/Localization/Basic.lean`.  The ready-made quotient
     localization equivalence is
     `Formalization.Books.Algebra.Unit09.localized_quotient_ring_equiv` in
     `Formalization/Books/Algebra/Unit09/Localization.lean` (instantiate its
     submonoid with `M.primeCompl` and its ideal with `M^n`).
  3. Apply those equivalences coordinatewise to obtain a ring equivalence
     between `AdicCompletion M A` and `AdicCompletion m S`; prove compatibility
     with transition maps using `AdicCompletion.transitionMap_ideal_mk` and
     with `evalₐ` by extensionality.
  4. Transport `completionMaximalIdeal`, every `completionKernel n`, their
     powers, and the image of the base maximal ideal across this equivalence.
     Transfer `infinitePolynomial_maximalIdeal_not_extended` and the kernel
     witness from `exists_completion_kernel_not_square`.  The witness and
     `completion_is_completeAsCompletionModule_iff_kernel_eq_pow` give failure
     of completeness over the completion.  Nonextension and the contrapositive
     of `completion_original_complete_implies_extended` give failure of
     completeness as an `S`-module, without transporting module completeness
     across two different base rings.
  5. Supply locality of the completion with `completion_isLocal m`, and assemble
     the existential tuple.  Taking `A` itself is a known dead end: the
     infinite polynomial ring is not local, which is why this localization
     bridge is essential to the book-level statement.
  -/
  sorry
