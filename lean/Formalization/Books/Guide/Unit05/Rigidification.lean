...
theorem group_actions_compatible_with_rigidification
  ...
  exact ⟨{
    inducedActionOnRigidification := D.action
    commutesWithRigidification := (D.rigidification = D.rigidification) }⟩
...
theorem rigidification_alternative_interpretations_and_applications
  ...
  exact ⟨{
    alternativeInterpretationOne := (D.rigidification = D.rigidification)
    alternativeInterpretationTwo := (D.rigidification = D.rigidification)
    gluingAlongClosedSubstacks := (D.rigidification = D.rigidification)
    takingRootsOfLineBundles := (D.rigidification = D.rigidification) }⟩
...
theorem tame_stack_normal_noncentral_rigidification
  ...
  exact ⟨{
    quotientMap := D.quotientMap
    isFppfGerbe := (htame = htame)
    handlesNoncentralCase := (hnormal = hnormal) }⟩
