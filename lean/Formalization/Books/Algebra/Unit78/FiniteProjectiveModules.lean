...
private lemma exists_not_mem_of_finite_submodules_of_local
    {R X : Type*} [CommRing R] [IsLocalRing R]
    [Infinite (IsLocalRing.ResidueField R)] [AddCommGroup X] [Module R X]
    {ι : Type*} [Finite ι] (p : ι → Submodule R X)
    (hQ : ∀ i, IsLocalRing.maximalIdeal R • (⊤ : Submodule R X) ≤ p i)
    (hp : ∀ i, p i ≠ ⊤) :
    ∃ x : X, ∀ i, x ∉ p i := by
  classical
  let Q : Submodule R X :=
  ...
  letI : IsScalarTower R (IsLocalRing.ResidueField R) (X ⧸ Q) := by
    ...
    exact inferInstanceAs
      (IsScalarTower R (R ⧸ IsLocalRing.maximalIdeal R)
        (X ⧸ (IsLocalRing.maximalIdeal R • (⊤ : Submodule R X))))
  let p' (i : ι) : Submodule (IsLocalRing.ResidueField R) (X ⧸ Q) :=
    { carrier := (p i).map Q.mkQ
      zero_mem' := by
        exact Submodule.mem_map_of_mem (p i).zero_mem
      add_mem' := by
        intro x y hx hy
        exact add_mem hx hy
      smul_mem' := by
        ...
        refine ⟨r • y, (p i).smul_mem r hy, ?_⟩
        rw [map_smul]
        simpa only [IsLocalRing.ResidueField.algebraMap_eq] using
          (IsScalarTower.algebraMap_smul (IsLocalRing.ResidueField R) r
            (Q.mkQ y)).symm }
  have hp' (i : ι) : p' i ≠ ⊤ := by
    intro h
    apply hp i
    have htop : (p i).map Q.mkQ = (⊤ : Submodule R (X ⧸ Q)) := by
      apply top_unique
      intro z hz
      have hz' : z ∈ p' i := by
        rw [h]
        exact (Submodule.mem_top : z ∈ (⊤ : Submodule
          (IsLocalRing.ResidueField R) (X ⧸ Q)))
      simpa [p'] using hz'
    have hsup : Q ⊔ p i = ⊤ := (Q.map_mkQ_eq_top (p i)).mp htop
    have hQ' : Q ≤ p i := by simpa [Q] using hQ i
    simpa [sup_eq_right.mpr hQ'] using hsup
  obtain ⟨z, hz⟩ := Submodule.exists_forall_notMem_of_forall_ne_top p' hp'
  obtain ⟨x, rfl⟩ := Q.mkQ_surjective z
  refine ⟨x, fun i hxi => hz i ?_⟩
