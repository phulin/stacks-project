...
/-- The associated evaluation functor is an equivalence of fibred categories over `C`. -/
theorem twoYonedaAssociatedFunctor_isFibredEquivalenceOver
  ...
  /- Proof plan: strictify the ordinary inverse of
  `twoYonedaAssociatedFunctor` over `C` by cartesian transport, then transport
  its unit and counit to vertical isomorphisms and package the inverse as a
  `FibredMorphism`. -/
  sorry
