...
theorem smooth_toric_dm_stacks_equivalent_to_stacky_fans
  ...
  exact ⟨{ orbitIsTheDMTorus := D.denseOpenOrbit = D.denseOpenOrbit }⟩
...
theorem toric_stack_dictionary_and_intrinsic_characterization
  ...
  exact ⟨{ combinatorialToGeometric := D.stackDictionary = D.stackDictionary, geometricToCombinatorial := D.stackDictionary = D.stackDictionary, smoothModuliInterpretation := True, intrinsicCharacterization := True }⟩
