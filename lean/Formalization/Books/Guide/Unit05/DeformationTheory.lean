...
theorem algebraization_unique_for_universal_deformation
  ...
  exact ⟨{ completionIso := W₁.formalCompletionIso ≪≫ W₂.formalCompletionIso.symm, identifiesFormalElements := huniversal = huniversal }⟩
...
theorem artin_axioms_are_stable_under_composition
  ...
  exact ⟨⟨hf.algebraizationAxiom = hf.algebraizationAxiom, hg.approximationAxiom = hg.approximationAxiom⟩⟩
