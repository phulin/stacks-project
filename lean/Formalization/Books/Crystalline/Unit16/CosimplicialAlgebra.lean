...
/-- One-step homotopy of cosimplicial module homomorphisms. -/
def CosimplicialModuleOneStepHomotopic {A : CosimplicialRing.{u}}
    {M N : CosimplicialModule.{u} A} (φ ψ : CosimplicialModuleHom M N) : Prop :=
  Nonempty (CosimplicialModuleHomotopy φ ψ)

/-- The source's homotopy relation, generated by one-step homotopies. -/
def CosimplicialModuleHomotopic {A : CosimplicialRing.{u}}
    {M N : CosimplicialModule.{u} A} (φ ψ : CosimplicialModuleHom M N) : Prop :=
  Relation.EqvGen (fun φ ψ : CosimplicialModuleHom M N =>
    CosimplicialModuleOneStepHomotopic φ ψ) φ ψ
...
structure CosimplicialTensorProductOperation (A : CosimplicialRing.{u}) where
  ...
  map_oneStepHomotopy : ∀ {M N L : CosimplicialModule.{u} A}
    {φ ψ : CosimplicialModuleHom M N},
    CosimplicialModuleOneStepHomotopic φ ψ →
      CosimplicialModuleOneStepHomotopic
        (map (L := L) φ) (map (L := L) ψ)
...
structure CosimplicialExteriorPowerOperation (A : CosimplicialRing.{u})
  ...
  map_component : ∀ {M N : CosimplicialModule.{u} A} (i : ℕ)
    (φ : CosimplicialModuleHom M N) (n : ℕ),
    (map i φ).app n = componentMap i n (φ.app n)
  map_oneStepHomotopy : ∀ {M N : CosimplicialModule.{u} A} (i : ℕ)
    {φ ψ : CosimplicialModuleHom M N},
    CosimplicialModuleOneStepHomotopic φ ψ →
      CosimplicialModuleOneStepHomotopic (map i φ) (map i ψ)
...
structure CosimplicialBaseChangeOperation {A B : CosimplicialRing.{u}}
  ...
  map_oneStepHomotopy : ∀ {M N : CosimplicialModule.{u} A}
    {φ ψ : CosimplicialModuleHom M N},
    CosimplicialModuleOneStepHomotopic φ ψ →
      CosimplicialModuleOneStepHomotopic (map φ) (map ψ)
