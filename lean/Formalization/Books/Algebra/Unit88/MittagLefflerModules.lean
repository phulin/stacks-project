...
theorem isMittagLefflerModule_of_restrictScalars
  ...
  have hP' : Module.FinitePresentation S (P' : Type (max v w)) := by
    infer_instance
  let φ : S →ₗ[S] (P : Type (max v w)) →ₗ[R] (M : Type (max v w)) :=
    { toFun := fun s =>
        { toFun := fun p => s • fP p
          map_add' := fun p q => by simp
          map_smul' := fun r p => by
            rw [map_smul]
            exact smul_comm s r (fP p) }
      map_add' := fun s t => by
        apply LinearMap.ext
        intro p
        simp [add_smul]
  ...
  let ι : (P : Type (max v w)) →ₗ[R] (P' : Type (max v w)) :=
    (TensorProduct.mk R S (P : Type (max v w))) 1
  have hι : (f'.restrictScalars R).comp ι = fP := by
    apply LinearMap.ext
    intro p
