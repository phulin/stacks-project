...
theorem Kresch_chow_groups_for_artin_stacks
  ...
  exact ⟨{
    agreesWithQuotientStackDefinition := (D.artin = D.artin)
    usualFunctorialProperties := (D.artin = D.artin) }⟩
...
theorem intrinsic_normal_cone_gives_virtual_fundamental_class
  ...
  exact ⟨{
    virtualClass := 0
    satisfiesVirtualClassAxioms :=
      (D.perfectObstructionTheory = D.perfectObstructionTheory) }⟩
