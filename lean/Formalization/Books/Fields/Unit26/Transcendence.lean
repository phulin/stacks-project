...
/-! ### The finite-conjugate/norm step

The analytic estimate imported above only supplies the exponentially small
approximants.  The arithmetic half of the Lindemann--Weierstrass argument
uses all embeddings of one fixed finite number field at once: their product is
an algebraic norm, hence a nonzero integer after the relevant integrality
normalization.  The following interfaces keep that step separate from the
analytic estimates and retain multiplicities of conjugates.
-/

noncomputable def finiteConjugateEmbeddings
    {F K A : Type*} [Field F] [Field K] [Field A]
    [Algebra F K] [Algebra F A] [FiniteDimensional F K] :
    Finset (K →ₐ[F] A) := Finset.univ

/-- The product over the finite conjugate embeddings is the algebra norm. -/
theorem finite_conjugate_product_eq_algebra_norm
    {F K A : Type*} [Field F] [Field K] [Field A]
    [Algebra F K] [Algebra F A] [FiniteDimensional F K]
    [Algebra.IsSeparable F K] [IsAlgClosed A] (x : K) :
    (finiteConjugateEmbeddings (F := F) (K := K) (A := A)).prod (fun σ => σ x) =
      algebraMap F A (Algebra.norm F x) := by
  simpa [finiteConjugateEmbeddings] using
    (Algebra.norm_eq_prod_embeddings F A x).symm

/-- A nonzero integral element has a nonzero integral conjugate norm. -/
theorem finite_conjugate_norm_is_nonzero_integer
    {K : Type*} [Field K] [Algebra ℚ K] [FiniteDimensional ℚ K]
    [Algebra.IsSeparable ℚ K] {x : K} (hx : IsIntegral ℤ x) (hx0 : x ≠ 0) :
    ∃ z : ℤ, z ≠ 0 ∧
      algebraMap ℤ ℂ z =
...
theorem finite_conjugate_norm_tends_to_zero_contradiction
    {K : Type*} [Field K] [Algebra ℚ K] [FiniteDimensional ℚ K]
    [Algebra.IsSeparable ℚ K] (x : ℕ → K)
    (hx : ∀ p, IsIntegral ℤ (x p)) (hx0 : ∀ p, x p ≠ 0)
    (hsmall : ∀ ε : ℝ, 0 < ε → ∃ p : ℕ,
      ‖(finiteConjugateEmbeddings (F := ℚ) (K := K) (A := ℂ)).prod
        (fun σ => σ (x p))‖ < ε) :
    False := by
...
theorem euler_number_transcendental_over_rationals :
  ...
  have h_not_injective :
      ¬Function.Injective
        (Polynomial.aeval eulerNumber : Polynomial ℚ →ₐ[ℚ] ℂ) := by
    intro hinj
    apply h
    intro p hp
    apply hinj
    simpa using hp
  have h_alg : IsAlgebraic ℚ eulerNumber :=
