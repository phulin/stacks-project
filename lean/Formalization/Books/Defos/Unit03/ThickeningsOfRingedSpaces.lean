...
/-- The structure rings of a ringed space are commutative on stalks.

The ambient `RingedSpace` API is `RingCat`-valued, whereas the ringed spaces
in the source chapter are commutatively ringed.  The stalkwise formulation is
the exact amount of commutativity used by the strict/cartesian criterion. -/
def HasCommutativeStructureStalks (X : RingedSpace.{v}) : Prop :=
  ∀ (x : X)
    (a b : TopCat.Presheaf.stalk (C := RingCat.{v}) X.structureSheaf.obj x),
    a * b = b * a
...
/- Proof roadmap for the `prove` stage:
   The `hcomm` hypothesis is essential, not cosmetic.  `RingedSpace` in
   `Formalization/Books/Sheaves/Unit25/Infrastructure.lean` is valued in
   `RingCat`, while the omitted source proof is for commutative rings.  For a
   surjection `R ⟶ B` and a map `R ⟶ A`, the pushout in `RingCat` quotients
   `A` by the *two-sided* ideal generated by the image of `ker (R ⟶ B)`;
   `Epi (inducedIdealMap M)` says that the generated left ideal is the whole
   kernel.  These agree when the stalk ring `A` is commutative.

   First isolate three small reusable helpers before this theorem.

   1. Prove a sheaf-module stalk criterion

        Epi p ↔ ∀ x, Function.Surjective
          ((sheafModuleStalkFunctor O x).map p).hom.

      Use `sheafModuleShortComplex` and
      `sheafModule_exact_iff_stalkwise` from
      `Formalization/Books/Modules/Unit03/AbelianCategory.lean`, applied to
      `p` followed by zero.  On each stalk finish with
      `ShortComplex.exact_iff_epi` and `ModuleCat.epi_iff_surjective`.
      This is the public two-way version of the private one-way lemma
      `stalkMap_surjective_of_epi` in
      `Formalization/Books/Modules/Unit04/Sections.lean`.

   2. Prove the elementary stalk-ring lemma.  For a commutative ring `A`, a
      commutative square in `RingCat.{v}`
      ...
      with `q.hom` surjective is a pushout iff the canonical extension-of-
      scalars map

        A ⊗[R] RingHom.ker q.hom ⟶ RingHom.ker s.hom,
        a ⊗ j ↦ a * p.hom j,

      is surjective.  Install the `CommRing A` structure from the supplied
      `∀ a b, a * b = b * a`.  Put
      `K := Ideal.map p.hom (RingHom.ker q.hom)`.  The tensor map has range
      `K`; prove this from `TensorProduct.lift.tmul` and the induction
      principle `Submodule.span_induction` for membership in `Ideal.map`.
      The pushout condition is
      equivalent to `K = RingHom.ker s.hom`: for the nontrivial inclusion,
      map `A` to `A ⧸ K`, descend that map through `q` with
      `RingHom.liftOfSurjective` from
      `Mathlib/RingTheory/Ideal/Maps.lean`, and use `IsPushout.desc` and its
      two fac equations.  Conversely, descend any compatible pair through
      `q`; equality of kernels makes its `A`-map factor uniquely through
      `s`.  Keep this lemma in `RingCat`, rather than replacing the ambient
      category by `CommRingCat`: the theorem's categorical pullback is in
      the former category.

   3. Prove a homeomorphic-ringed-square bridge.  If the horizontal maps
      `i : X ⟶ X'` and `t : S ⟶ S'` have homeomorphic continuous maps, a
      commutative ringed-space square is a pullback iff, for every `x : X`,
      the square of stalk ring maps

        O_{S',t(f x)} ⟶ O_{S,f x}
              ↓              ↓
        O_{X',i x}     ⟶ O_{X,x}

      is a pushout in `RingCat.{v}`.  The four arrows are
      `algebraicFMapStalkMap` applied to `t.sharp`, `f'.sharp`, `f.sharp`,
      and `i.sharp`; its commutativity is
      `algebraicFMapComp_stalkMap` from
      `Formalization/Books/Sheaves/Unit23/Infrastructure.lean`, rewritten by
      `M.commutes`.  Use `TopCat.isIso_iff_isHomeomorph` to install `IsIso`
      for the horizontal continuous maps.  For a direct proof, apply
      `IsPullback.mk'` (from
      `Mathlib/CategoryTheory/Limits/Shapes/Pullback/IsPullback/Basic.lean`):
      the topological lift is `a.continuous ≫ inv i.continuous`; the stalk
      pushouts uniquely define its `sharp`, and the sheaf condition glues the
      locally defined maps.  Equality of ringed-space morphisms follows
      first on continuous maps and then on all stalk maps.  For the reverse
      implication test the ringed-space universal property against a
      one-point ringed space.  Reuse `onePointRingedSpace` from
      `Formalization/Books/Modules/Unit10/QuasiCoherent.lean`; add the small
      equivalence between maps from this point over `x` and ring maps out of
      `X.structureSheaf.obj.stalk x`, built with `TopCat.Presheaf.germ`.
      This avoids constructing a global quotient ringed space.

   Now fix `x : M.X`, set `x' := M.i.continuous x`, and instantiate the
   ring lemma with the four stalk maps in helper 3.  Surjectivity of the top
   map is exactly
   `M.t_isThickening.structureSheaf_surjective (M.f.continuous x)`.
   Commutativity of the lower-left ring is `hcomm x'`.

   Identify the tensor kernel map with the stalk of `inducedIdealMap M` as
   follows.  Let

     j := (ringedSpaceModuleFMapPullbackHomEquiv M.f'
       M.baseIdeal.carrier (SheafOfModules.unit M.X'.structureSheaf)).symm
       (M.baseIdeal.inclusion ≫
         SheafOfModules.unitToPushforwardObjUnit M.f'.sharp).

   Prove the important intermediate equation

     inducedIdealMap M ≫ M.sourceIdeal.inclusion = j.

   Apply the adjunction Hom equivalence to both sides, rewrite with
   `Adjunction.homEquiv_naturality_right`, unfold `inducedIdealMap`, and use
   `inducedIdealFMap_fac M`.  On the stalk `x'`, use
   `ringedSpaceModulePullback_stalk_formula M.f' M.baseIdeal.carrier x'`
   from `Formalization/Books/Sheaves/Unit26/Infrastructure.lean` and the
   defining formulas for `moduleStalkAddMap`/`ringedSpaceModuleFMapStalkMap`.
   The displayed equation and monicity of `kernel.ι` identify this stalk map
   with `a ⊗ j ↦ a * f'^sharp_x(j)`, i.e. the map in helper 2.

   Finally use helper 1 to turn `MorphismOfThickenings.IsStrict M` into
   surjectivity of all these stalk maps, helper 2 pointwise to exchange that
   for the stalk pushout condition, and helper 3 to exchange the latter for
   the required `IsPullback`.  Reindex points of `M.X'` along
   `M.i_isThickening.underlying_homeomorph.surjective` in the strictness
   direction.

   Known false route: do not remove `hcomm`.  On one-point spaces take a
   field `k`, `Q = M₂(k)`, the square-zero extension `A' = Q ⋉ Q`, and map
   `k[ε]/(ε²) ⟶ A'` by `ε ↦ (0, e₁₁)`.  The pushout with `k` is `Q`, since
   the two-sided ideal `Q e₁₁ Q` is all of the square-zero kernel, while the
   induced left ideal is only `Q e₁₁`.  Thus the ringed-space square is
   cartesian but the induced ideal map is not epic. -/
theorem strict_iff_cartesian
    (M : MorphismOfThickenings)
    (hcomm : HasCommutativeStructureStalks M.X')
    [((SheafOfModules.pushforward (F := Opens.map M.f'.continuous)
      M.f'.sharp).IsRightAdjoint)] :
